//! Atipicial Contract Methods Example
//!
//! This example demonstrates how to interact with contract methods on Atipicial,
//! including reading contract state, invoking functions, and handling different
//! parameter types and return values.

use atipicial::{atipicial_clients::APITrait, atipicial_types::ContractParameter, prelude::*};
use std::str::FromStr;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	println!("🔧 Atipicial Contract Methods Example");
	println!("==================================\n");
...
async fn query_contract_info(
	...
	match client.get_contract_state(gas_hash).await {
		Ok(state) => {
			...
			println!("      AEF checksum: 0x{:x}", state.aef.checksum);
