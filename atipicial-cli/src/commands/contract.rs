...
use atipicial::{
	...
	atipicial_contract::PolicyContract,
	atipicial_protocol::AccountTrait,
	atipicial_types::{ContractManifest, AefFile},
	prelude::*,
};
...
pub enum ContractCommands {
	...
	Native {
		/// Native contract name (for example AtipicialCoin, GasToken, PolicyContract)
		#[arg(short, long)]
		name: String,
	},
	...
	Update {
		...
		/// Path to the new contract file (.aef)
		#[arg(short, long)]
		aef: PathBuf,
...
pub async fn handle_contract_command(
	...
	match args.command {
		...
		ContractCommands::Deploy { aef, manifest, account } => {
			deploy_contract(aef, manifest, account, state).await
		},
		ContractCommands::Update { script_hash, aef, manifest, account } => {
			update_contract(script_hash, aef, manifest, account, state).await
		},
...
async fn deploy_contract(
	...
	if !aef_path.exists() {
		print_error(&format!("AEF file not found: {:?}", aef_path));
		return Err(CliError::Input(format!("AEF file not found: {:?}", aef_path)));
	}
	...
	// Read AEF and manifest files
	let aef_bytes = std::fs::read(&aef_path).map_err(|e| CliError::Io(e))?;
	let manifest_json = std::fs::read_to_string(&manifest_path).map_err(|e| CliError::Io(e))?;

	// Parse AEF and manifest
	let _aef = AefFile::deserialize(&aef_bytes)
		.map_err(|e| CliError::Input(format!("Failed to parse AEF file: {}", e)))?;
	...
	// Get system fee
	let params =
		vec![ContractParameter::byte_array(aef_bytes), ContractParameter::string(manifest_json)];
	...
	let mut tx_builder: TransactionBuilder<'_, atipicial::atipicial_clients::HttpProvider> =
	...
	let mut encoder = atipicial::codec::Encoder::new();
...
async fn show_policy(state: &mut crate::commands::wallet::CliState) -> Result<(), CliError> {
	...
	let pico_factor = match policy.get_exec_pico_fee_factor().await {
		...
		Err(_) => "Not supported (Atipicial 3.9+)".to_string(),
	};
