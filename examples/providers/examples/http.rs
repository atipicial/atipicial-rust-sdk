//! The Http transport is used to send JSON-RPC requests over HTTP to an Atipicial node.
//! This is the most basic connection to a node.

use atipicial::atipicial_clients::APITrait;
use std::sync::Arc;

const RPC_URL: &str = "https://testnet1.atipicial.com:443/";
...
async fn create_instance() -> eyre::Result<()> {
	// Create an HTTP provider for Atipicial TestNet
	let provider = atipicial::atipicial_clients::HttpProvider::new(RPC_URL)?;
