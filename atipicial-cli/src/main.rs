...
use commands::{
	...
	atipicialfs::{handle_atipicialfs_command, AtipicialFsArgs},
...
/// Atipicial CLI - A comprehensive command-line interface for the Atipicial blockchain
///
/// This tool provides access to all Atipicial blockchain functionality including
...
#[clap(
	...
	about = "A comprehensive CLI for Atipicial blockchain operations",
	long_about = "Atipicial CLI provides a complete command-line interface for interacting with the Atipicial blockchain. \
...
enum Commands {
	...
	#[command(about = "Initialize Atipicial CLI configuration")]
	Init {
	...
	#[command(about = "Manage Atipicial wallets and accounts")]
	Wallet(WalletArgs),
	...
	/// AtipicialFs file storage operations
	#[command(about = "Decentralized file storage on AtipicialFs")]
	AtipicialFs(AtipicialFsArgs),
	...
	/// AtipicialFs operations
	#[command(about = "AtipicialFs file storage operations")]
	...
	/// Generate a new Atipicial project from templates
	#[command(about = "Generate a new Atipicial project from pre-built templates")]
	Generate {
		/// Template to use (basic-dapp, aep17-token, nft, defi, oracle)
...
async fn handle_init_command(
	...
	print_info("🚀 Initializing Atipicial CLI configuration...");
	...
	print_info("💡 Use 'atipicial-cli config' to view current settings");
	print_info("💡 Use 'atipicial-cli wallet create' to create your first wallet");
...
fn handle_version_command(format: OutputFormat) -> Result<(), CliError> {
	match format {
		...
		OutputFormat::Table => {
			println!("{}", "Atipicial CLI".bright_green().bold());
			...
			println!("Repository: {}", "https://github.com/R3E-Network/AtipicialRust".bright_blue());
		},
	}
	Ok(())
}
...
async fn handle_config_command(
	...
	if config_path.exists() {
		...
		print_warning("No configuration file found. Run 'atipicial-cli init' to create one.");
	}
...
async fn main() -> Result<(), CliError> {
	...
	if !matches!(cli.command, Commands::Version | Commands::Config { .. }) {
		println!("{}", "🔷 Atipicial CLI".bright_green().bold());
		println!("{}", "Atipicial Blockchain Interface".bright_blue());
		println!();
	}
		...
		let mut networks: Vec<NetworkConfig> = cfg
			...
			.map(|n| {
				...
				let rpc_url = match (network_type.as_str(), n.rpc_url.as_str()) {
					("mainnet", "http://seed1.atipicial.com:10333") => {
						default_networks[0].rpc_url.clone()
					},
					("testnet", "https://testnet1.atipicial.com:443") => {
	...
	match cli.command {
		...
		Commands::AtipicialFs(args) => handle_atipicialfs_command(args, &mut state).await,
		...
		Commands::Generate { template, name, dir, list } => {
			if list {
				...
				let template_type = match template.as_deref() {
					...
					Some("aep17-token") => generator::ProjectTemplate::Aep17Token,
