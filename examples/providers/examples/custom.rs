/// Atipicial Custom Provider Example
///
/// This example demonstrates how to create custom provider abstractions
/// for Atipicial blockchain interactions with specialized functionality.
use std::collections::HashMap;
use std::time::Duration;

/// Custom provider trait for Atipicial operations
trait CustomAtipicialProvider {
	fn name(&self) -> &str;
...
impl Default for ProviderConfig {
	fn default() -> Self {
		Self {
			...
			enable_caching: true,
			endpoints: vec![
				"https://mainnet1.atipicial.coz.io:443".to_string(),
				"https://mainnet2.atipicial.coz.io:443".to_string(),
			],
		}
	}
}
...
impl CustomAtipicialProvider for LoadBalancingProvider {
	fn name(&self) -> &str {
		&self.name
	}
...
impl CustomAtipicialProvider for CachingProvider {
	fn name(&self) -> &str {
		&self.name
	}
...
impl CustomAtipicialProvider for MonitoringProvider {
	fn name(&self) -> &str {
		&self.name
	}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	println!("🔧 Atipicial Custom Provider Example");
	println!("=================================");
	...
	let endpoints = vec![
		"https://mainnet1.atipicial.coz.io:443".to_string(),
		"https://mainnet2.atipicial.coz.io:443".to_string(),
		"https://mainnet3.atipicial.coz.io:443".to_string(),
	];
	...
	load_balancer.mark_endpoint_health("https://mainnet2.atipicial.coz.io:443", false);
	...
	let configs = vec![
		...
		(
			...
			ProviderConfig {
				...
				max_retries: 0,
				enable_caching: false,
				endpoints: vec!["https://testnet.atipicial.com:443".to_string()],
			},
		),
	];
	...
	println!("       .build();");
	println!();
	println!("   // Use with Atipicial client");
	println!("   let client = AtipicialClient::with_provider(provider);");
