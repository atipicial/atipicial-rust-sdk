...
/// Atipicial X EVM transaction for interacting with the Atipicial X EVM-compatible chain
...
impl AtipicialXTransaction {
	/// Creates a new AtipicialXTransaction instance
	///
	/// # Arguments
	///
	...
	/// # Returns
	///
	/// A new AtipicialXTransaction instance
	pub fn new(
		to: Option<H160>,
		data: Vec<u8>,
		value: u64,
	...
	///
	/// This additive path preserves the legacy [`AtipicialXTransaction`] serialization and accessors
...
impl From<AtipicialXTransaction> for TransactionRequest {
	fn from(tx: AtipicialXTransaction) -> Self {
...
mod tests {
	...
	fn test_into_alloy_request() {
		let tx = AtipicialXTransaction::new(None, vec![1, 2, 3], 1000, 21000, 1_000_000_000);
		...
		assert_eq!(serde_json::from_value::<AtipicialXTransaction>(serialized).unwrap().value(), 1000);
	...
	fn preserves_values_above_u64() {
		...
		let request =
			AtipicialXTransaction::build_alloy_request(None, Vec::new(), value, 21_000, 1_000_000_000);
