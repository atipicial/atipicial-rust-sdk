...
# Atipicial CLI Integration Tests

This directory contains integration tests for the `atipicial-cli` commands. These tests verify that the CLI commands work correctly by executing the actual binary and checking its output.
...
These tests execute the actual `atipicial-cli` binary, so they require:

1. The binary to be built before testing
2. Internet connectivity for tests that interact with the Atipicial blockchain
