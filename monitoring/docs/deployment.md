...
# AtipicialRust Monitoring Deployment Guide
...
helm install prometheus prometheus-community/kube-prometheus-stack \
  ...
  --set grafana.adminPassword=atipicialrust-admin \
...
### 5. Configure AtipicialRust Application

```yaml
# atipicialrust-deployment.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: atipicialrust
        image: your-registry/atipicialrust:latest
        env:
        - name: ATC_METRICS_ENABLED
          value: "true"
        - name: ATC_METRICS_PORT
          value: "9090"
        - name: ATC_TRACING_ENABLED
          value: "true"
        - name: ATC_TRACING_ENDPOINT
          value: "http://otel-collector.monitoring:4317"
        - name: ATC_LOG_LEVEL
          value: "info"
        - name: ATC_HEALTH_CHECK_ENABLED
          value: "true"
        - name: ATC_HEALTH_CHECK_PORT
          value: "8080"
        ports:
        - name: metrics
          containerPort: 9090
...
spec:
  ...
  - type: Pods
    pods:
      metric:
        name: atipicial_transactions_per_second
      target:
        type: AverageValue
        averageValue: "100"
```
...
spec:
  rules:
  - host: grafana.atipicialrust.example.com
    http:
      paths:
      - path: /
        pathType: Prefix
...
kubectl exec -n default atipicialrust-pod -- nc -zv otel-collector.monitoring 4317
```
