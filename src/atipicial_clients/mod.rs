...
//! # Atipicial Clients
//!
//! Client interfaces for interacting with Atipicial blockchain nodes.
//!
//! ## Overview
//!
//! The atipicial_clients module provides a comprehensive set of client interfaces for connecting to
//! and interacting with Atipicial blockchain nodes. It includes:
//!
//! - RPC clients for making JSON-RPC calls to Atipicial nodes
...
//! ## Examples
//!
//! ### Connecting to a Atipicial node using HTTP
//!
//! ```rust,no_run
...
//!     // Create an HTTP provider connected to a Atipicial TestNet node
...
//! ```ignore
//! # #[cfg(feature = "ws")]
//! use atipicial::atipicial_clients::{Ws, RpcClient, APITrait};
//!
//! # #[cfg(feature = "ws")]
//! #[tokio::main]
//! async fn main() -> Result<(), Box<dyn std::error::Error>> {
//!     // Connect to a Atipicial node using WebSocket
...
pub use rpc::*;
#[deprecated(
	note = "Use `HttpProvider::new(...)`/`RpcClient::new(...)` or the high-level `sdk::Atipicial` builder instead.",
	since = "1.0.1"
)]
...
fn default_http_provider_client() -> RpcClient<Http> {
	let url =
		url::Url::parse(AtipicialConstants::SEED_1).expect("AtipicialConstants::SEED_1 must be a valid URL");
...
pub fn try_http_provider_from_env() -> Result<RpcClient<Http>, ProviderError> {
	let endpoint = std::env::var("ENDPOINT").unwrap_or_else(|_| AtipicialConstants::SEED_1.to_string());
...
/// or the high-level `sdk::Atipicial` builder.
mod test_provider {
	...
	const MAINNET_ENDPOINTS: &[&str] = &[
		AtipicialConstants::SEED_1,
		AtipicialConstants::SEED_2,
		AtipicialConstants::SEED_3,
		AtipicialConstants::SEED_4,
		AtipicialConstants::SEED_5,
	];
	const TESTNET_ENDPOINTS: &[&str] =
		&["https://testnet1.atipicial.coz.io:443", "https://testnet2.atipicial.coz.io:443"];
	...
	impl TestProvider {
		...
		pub fn url(&self) -> String {
			...
			match endpoint {
				...
				None => {
					...
					AtipicialConstants::SEED_1.to_string()
				},
			}
		}
		...
		pub async fn ws(&self) -> Result<RpcClient<Ws>, ProviderError> {
			...
			let mut url = url::Url::parse(&url_str).unwrap_or_else(|e| {
				...
				url::Url::parse(AtipicialConstants::SEED_1)
					.expect("AtipicialConstants::SEED_1 must be a valid URL")
			});
