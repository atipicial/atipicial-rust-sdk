...
AtipicialRust exposes compile gates and scaffolding for Intel SGX (Software Guard Extensions). Treat this as an integration starting point, not certified enclave support. The general SDK still depends on `std` for normal host targets, and `no_std` is not certified for arbitrary embedded or WASM targets.
...
5. [Usage](#usage)
6. [Security Beaefits](#security-beaefits)
...
# Cargo.toml
[dependencies]
atipicial = { version = "3.0.0", features = ["sgx", "no_std"] }
...
enclave {
    ...
    trusted {
        public sgx_status_t atipicial_sign_transaction(
    ...
    untrusted {
        sgx_status_t ocall_atipicial_rpc_request(
...
extern crate atipicial;

use atipicial::atipicial_sgx::prelude::*;
use atipicial::atipicial_sgx::crypto::SgxCrypto;
use sgx_types::*;

#[no_mangle]
pub extern "C" fn sign_atipicial_transaction(
    data: *const u8,
    data_len: usize,
    signature: *mut u8,
) -> sgx_status_t {
...
```rust
use atipicial::atipicial_sgx::crypto::SgxKeyManager;
use atipicial::atipicial_sgx::storage::SecureStorage;
...
```rust
use atipicial::atipicial_sgx::attestation::RemoteAttestation;
...
```rust
use atipicial::atipicial_sgx::networking::{SgxNetworking, SecureChannel};
...
- [AtipicialRust SGX Examples](https://github.com/r3e-network/atipicial-rust-sdk/tree/master/examples/sgx_enclave)

## Summary

SGX support in AtipicialRust v2.0.0 provides:
