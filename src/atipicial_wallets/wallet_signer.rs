...
use crate::{
	atipicial_builder::{Transaction, TransactionError},
	atipicial_clients::JsonRpcProvider,
	atipicial_crypto::HashableForVec,
	atipicial_types::Address,
	atipicial_wallets::WalletError,
};

/// A Atipicial private-public key pair which can be used for signing messages.
///
/// # Examples
///
...
impl<D: PrehashSigner<Signature>> WalletSigner<D> {
	/// Creates a new `WalletSigner` instance using an external `Signer` and associated Atipicial address.
	///
	/// # Arguments
	///
	/// * `signer` - An implementation of the `PrehashSigner` trait capable of signing messages.
	/// * `address` - The Atipicial address associated with the signer's public key.
	///
	/// # Returns
	///
