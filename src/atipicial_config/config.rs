...
impl AtipicialNetwork {
	pub fn to_magic(&self) -> u32 {
		match self {
			AtipicialNetwork::MainNet => 0x334f454e,
			AtipicialNetwork::TestNet => 0x74746e41,
			AtipicialNetwork::PrivateNet => 0x4e454e,
		}
	}

	pub fn from_magic(magic: u32) -> Option<AtipicialNetwork> {
		match magic {
			0x334f454e => Some(AtipicialNetwork::MainNet),
			0x74746e41 => Some(AtipicialNetwork::TestNet),
			0x4e454e => Some(AtipicialNetwork::PrivateNet),
			_ => None,
		}
	}
}
...
lazy_static! {
	pub static ref ATCCONFIG: Mutex<AtipicialConfig> = Mutex::new(AtipicialConfig::default());
}

/// Acquires the global [`ATCCONFIG`] lock, recovering from poison if a
...
pub fn atipicial_config_lock() -> MutexGuard<'static, AtipicialConfig> {
	ATCCONFIG.lock().unwrap_or_else(|e| {
		tracing::warn!("ATCCONFIG mutex poisoned; recovering inner state");
		e.into_inner()
	})
}

impl Hash for AtipicialConfig {
...
impl Default for AtipicialConfig {
	fn default() -> Self {
...
impl AtipicialConfig {
	pub fn new(json_config: &str) -> Result<Self, serde_json::Error> {
		let mut config: AtipicialConfig = serde_json::from_str(json_config)?;
	...
	/// Creates a configuration for Atipicial Testnet (T5).
	pub fn testnet() -> Self {
		let mut hardforks = HashMap::new();
		...
		AtipicialConfig {
			...
			seed_list: vec![
				"seed1t5.atipicial.com:20333".to_string(),
				"seed2t5.atipicial.com:20333".to_string(),
				"seed3t5.atipicial.com:20333".to_string(),
				"seed4t5.atipicial.com:20333".to_string(),
				"seed5t5.atipicial.com:20333".to_string(),
			],
			nns_resolver: H160::from_slice(&hex!("50ac1c37690cc2cfc594472833cf57505d5f46de")),
			allows_transmission_on_fault: false,
		}
