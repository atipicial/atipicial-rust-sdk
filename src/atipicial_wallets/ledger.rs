...
use crate::{
	builder::Transaction, atipicial_clients::JsonRpcProvider, atipicial_wallets::WalletError, Address,
	ScriptHashExtension,
};
...
/// Atipicial APDU commands for Ledger devices.
mod apdu {
	use coins_ledger::common::APDUCommand;

	/// APDU command to get the Atipicial address for a given derivation path.
	...
	/// APDU command to sign a Atipicial transaction.
	...
	/// APDU command to sign a Atipicial message.
...
/// Represents a hierarchical deterministic path for Atipicial accounts.
#[derive(Debug, Clone)]
pub enum HDPath {
...
/// A Ledger hardware wallet signer for Atipicial.
...
impl<T: LedgerAsync> LedgerWallet<T> {
	...
	/// Gets the Atipicial address for the current derivation path.
	pub async fn get_address(&self) -> Result<Address, WalletError> {
		...
		// The response data contains the public key and the Atipicial address
		...
		// Convert the public key to a Atipicial address
		let address =
