use crate::{
	...
	deserialize_map,
	atipicial_contract::Role,
	atipicial_crypto::utils::{FromBase64String, FromHexString, ToHexString},
	atipicial_types::script_hash::ScriptHashExtension,
	serialize_map, Base64Encode, ContractParameterType, NNSName, AefFile, ValueExtension,
};
use getset::Getters;
use hex;
...
impl From<&AefFile> for ContractParameter {
	fn from(value: &AefFile) -> Self {
		Self::try_from_aef_file(value).unwrap_or_else(|err| {
			tracing::warn!(
				error = %err,
				"Failed to convert AEF file to contract parameter via safe path; falling back to legacy serializer"
			);
...
mod tests {
	...
	use crate::{
		crypto::Secp256r1PublicKey, atipicial_types::ContractParameterMap, AefFile, ValueExtension,
	};
	use atipicial::prelude::{ContractParameter, ContractParameterType};

	#[test]
	fn test_try_from_aef_file_ref_rejects_invalid_aef() {
		let aef = AefFile::new(Some("x".repeat(65)), String::new(), vec![1, 2, 3], vec![0; 4]);

		assert!(matches!(
			ContractParameter::try_from_aef_file(&aef),
			Err(crate::atipicial_types::TypeError::InvalidEncoding(message))
				if message.contains("compiler")
		));
	}

	#[test]
	fn test_from_aef_file_ref_repairs_checksum_via_legacy_wrapper() {
		let mut aef = AefFile::new(
			Some("test-compiler".to_string()),
			String::new(),
			vec![0x11, 0x40],
			vec![0; 4],
		);
		aef.checksum = vec![0; 4];
		...
		assert_eq!(
			ContractParameter::from(&aef),
			ContractParameter::byte_array(canonical_aef.try_to_array().unwrap())
		);
	}

	#[test]
	fn test_try_from_aef_file_owned_matches_legacy_for_valid_aef() {
		let aef = AefFile::new(
			Some("test-compiler".to_string()),
			String::new(),
			vec![0x11, 0x40],
		...
		assert_eq!(
			ContractParameter::try_from_aef_file(&aef).unwrap(),
			ContractParameter::from(&aef)
		);
	}
	...
	fn test_nested_array() {
		let nested_params = vec![
			...
			ContractParameter::string("atipicialw3j:)".to_string()),
	...
	fn test_bytes_from_string() {
		let param = ContractParameter::byte_array("Atipicial".as_bytes().to_vec());
		// assert_param(&param, b"Atipicial", ContractParameterType::ByteArray);
		assert_eq!(param.typ, ContractParameterType::ByteArray);
		assert_eq!(
			param
				.value
	...
	fn contract_parameter_to_value_is_structured_json() {
		let parameter = ContractParameter::string("atipicial".to_string());
