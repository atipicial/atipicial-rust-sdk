...
impl AtipicialConstants {
	...
	/// at runtime by setting the `ATIPICIAL_MAX_RPC_MESSAGE_SIZE` environment variable to a positive
	/// integer.
	#[cfg(not(all(feature = "sgx", target_env = "sgx")))]
	pub fn max_rpc_message_size() -> usize {
		std::env::var("ATIPICIAL_MAX_RPC_MESSAGE_SIZE")
			.ok()
	...
	/// runtime by setting the `ATIPICIAL_RPC_TIMEOUT_SECS` environment variable to a positive integer.
	#[cfg(not(all(feature = "sgx", target_env = "sgx")))]
	pub fn rpc_request_timeout() -> Option<core::time::Duration> {
		std::env::var("ATIPICIAL_RPC_TIMEOUT_SECS")
			.ok()
...
impl Default for AtipicialConstants {
	fn default() -> Self {
		Self::new()
	}
}
...
mod tests {
	...
	fn rpc_request_timeout_parses_env_var() {
		with_env_var("ATIPICIAL_RPC_TIMEOUT_SECS", None, || {
			assert_eq!(AtipicialConstants::rpc_request_timeout(), None);
		});

		with_env_var("ATIPICIAL_RPC_TIMEOUT_SECS", Some("5"), || {
			assert_eq!(
				AtipicialConstants::rpc_request_timeout(),
				Some(core::time::Duration::from_secs(5))
			);
		});

		with_env_var("ATIPICIAL_RPC_TIMEOUT_SECS", Some("0"), || {
			assert_eq!(AtipicialConstants::rpc_request_timeout(), None);
		});

		with_env_var("ATIPICIAL_RPC_TIMEOUT_SECS", Some("not-a-number"), || {
			assert_eq!(AtipicialConstants::rpc_request_timeout(), None);
		});
	}
}
