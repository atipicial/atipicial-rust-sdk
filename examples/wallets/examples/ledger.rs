/// Atipicial Ledger Hardware Wallet Integration Example
///
/// This example demonstrates a simulation of Ledger hardware wallet integration
/// for Atipicial, including device discovery, address derivation, and transaction signing.
...
impl LedgerDevice {
	...
	fn new() -> Self {
		Self {
			...
			atipicial_app_version: "1.0.6".to_string(),
			connected: true,
			unlocked: false,
		}
	}
	...
	/// Simulate getting Atipicial app info
	fn get_atipicial_app_info(&self) -> Result<HashMap<String, String>, Box<dyn std::error::Error>> {
		if !self.connected {
			return Err("Device not connected".into());
		}

		let mut info = HashMap::new();
		info.insert("name".to_string(), "Atipicial".to_string());
		info.insert("version".to_string(), self.atipicial_app_version.clone());
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	println!("🔐 Atipicial Ledger Hardware Wallet Integration Example");
	println!("===================================================");
	...
	// 2. Check Atipicial app
	println!("\n2️⃣ Checking Atipicial app on device...");
	match ledger.get_atipicial_app_info() {
		Ok(app_info) => {
			println!("   ✅ Atipicial app detected");
			for (key, value) in app_info {
				println!("   📋 {key}: {value}");
			}
		},
		Err(e) => {
			println!("   ❌ Atipicial app not found: {e}");
			println!("   💡 Please install the Atipicial app on your Ledger device");
		},
	}
	...
	// 4. Derive Atipicial addresses
	println!("\n4️⃣ Deriving Atipicial addresses...");
	let bip44_paths = vec![
		"m/44'/888'/0'/0/0", // First Atipicial address
		"m/44'/888'/0'/0/1", // Second Atipicial address
		"m/44'/888'/0'/0/2", // Third Atipicial address
	];
	...
	// 5. Connect to Atipicial network
	println!("\n5️⃣ Connecting to Atipicial TestNet...");
	...
	println!("   hardware wallet security with Atipicial applications");
