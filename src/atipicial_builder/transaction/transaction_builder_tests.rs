...
mod tests {
	...
	use crate::{
		...
		config::{AtipicialConstants, TestConstants},
		crypto::{KeyPair, Secp256r1PrivateKey},
		atipicial_builder::{OracleResponse, OracleResponseCode, GAS_TOKEN_HASH},
		atipicial_clients::{APITrait, MockClient, MockProvider, RpcClient},
		atipicial_crypto::utils::ToHexString,
		atipicial_protocol::{
			Account, AccountTrait, ApplicationLog, AtipicialProtocol, AtipicialVersion, RawTransaction,
		},
	...
	use atipicial::{
		...
		config::{AtipicialConfig, ATCCONFIG},
	};
	...
	async fn test_invoke_script() {
		...
		let script = ScriptBuilder::new()
			.contract_call(
				&ScriptHashExtension::from_hex(TestConstants::ATC_TOKEN_HASH).unwrap(),
				"symbol",
				&[],
				None,
			)
		...
		let mock_provider = Arc::new(Mutex::new(MockClient::new().await));
		{
			...
			let mut mock_provider_guard = mock_provider_guard
				.mock_response_with_file_ignore_param(
					"invokescript",
					"invokescript_symbol_atipicial.json",
				)
				.await;
			mock_provider_guard.mount_mocks().await;
		}
		...
		if let Some(item) = response.stack.get(0) {
			if let Some(value) = item.as_string() {
				assert_eq!(value, "ATC", "Unexpected value in response");
			} else {
	...
	async fn test_send_invoke_function() {
		...
		let script = ScriptBuilder::new()
			.contract_call(
				&H160::from_str(TestConstants::ATC_TOKEN_HASH).unwrap(),
				"transfer",
				&[
	...
	async fn test_fail_adding_more_than_max_attributes_to_tx_just_attributes() {
		let attrs: Vec<TransactionAttribute> = (0..=AtipicialConstants::MAX_TRANSACTION_ATTRIBUTES)
	...
	async fn test_fail_adding_more_than_max_attributes_to_tx_attributes_and_signers() {
		...
		.unwrap();
		let attrs: Vec<TransactionAttribute> = (0..=AtipicialConstants::MAX_TRANSACTION_ATTRIBUTES - 3)
			.map(|_| TransactionAttribute::HighPriority)
			.collect();
		assert_eq!(
			tb.add_attributes(attrs),
	...
	async fn test_fail_adding_more_than_max_attributes_to_tx_signers() {
		...
		let signers: Vec<Signer> = (0..AtipicialConstants::MAX_TRANSACTION_ATTRIBUTES)
			.map(|_| AccountSigner::called_by_entry(&Account::create().unwrap()).unwrap().into())
			.collect();
		assert!(signers.len() + 1 > AtipicialConstants::MAX_TRANSACTION_ATTRIBUTES.try_into().unwrap());
		assert_eq!(
	...
	#[tokio::test]
	async fn test_transfer_atipicial_from_normal_account() {
		let mock_provider = Arc::new(Mutex::new(MockClient::new().await));
		{
			let mut mock_provider_guard = mock_provider.lock().await; // Lock the mock_provider once
			let mut mock_provider_guard = mock_provider_guard
		...
		let mock_provider = Arc::new(Mutex::new(MockClient::new().await));
		{
			...
			let mut mock_provider_guard = mock_provider_guard
				.mock_response_with_file(
					"invokefunction",
					"invokefunction_transfer_atipicial.json",
					json!([
						TestConstants::ATC_TOKEN_HASH,
						"transfer",
						vec![
							ContractParameter::from(ACCOUNT1.address_or_scripthash().script_hash()),
							ContractParameter::from(
