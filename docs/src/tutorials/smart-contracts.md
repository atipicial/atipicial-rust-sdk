...
This tutorial covers working with smart contracts on the Atipicial blockchain using the AtipicialRust SDK.

## Understanding Atipicial Smart Contracts

Atipicial smart contracts are written in a variety of languages including C#, Python, Go, and TypeScript, and are compiled to AtipicialVM bytecode. The AtipicialRust SDK provides tools for deploying and interacting with these contracts.

## Deploying a Smart Contract

To deploy a smart contract, you need the contract's AEF (Atipicial Executable Format) file and its manifest:
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Read the AEF file and manifest
    ...
    let transaction = TransactionBuilder::new()
        ...
        .deploy_contract(&aef_bytes, &manifest_bytes)
        .sign(account)?
        .build();
...
AEP-17 is Atipicial's token standard, similar to Ethereum's ERC-20. The AtipicialRust SDK provides a convenient way to work with AEP-17 tokens:
...
Atipicial smart contracts can emit events that you can listen for:
