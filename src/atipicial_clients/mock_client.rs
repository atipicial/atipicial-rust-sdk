use crate::{
	...
	atipicial_protocol::{Account, AccountTrait, ApplicationLog, AtipicialVersion, RawTransaction},
...
impl MockClient {
	...
	pub async fn mock_response_error(&mut self, error: serde_json::Value) {
		let json_error: crate::atipicial_clients::JsonRpcError =
			serde_json::from_value(error).unwrap_or(crate::atipicial_clients::JsonRpcError {
				code: -32000,
	...
	pub async fn mock_get_version(&mut self, result: AtipicialVersion) -> &mut Self {
...
enum JsonRpcFixture {
	Result(Value),
	Error(crate::atipicial_clients::JsonRpcError),
}
...
async fn load_jsonrpc_response(response_file: &str) -> JsonRpcFixture {
	...
	if let Some(error) = parsed.get("error") {
		let json_error: crate::atipicial_clients::JsonRpcError = serde_json::from_value(error.clone())
			.unwrap_or(crate::atipicial_clients::JsonRpcError {
				code: -32000,
				message: "mock error".to_string(),
				data: None,
			});
