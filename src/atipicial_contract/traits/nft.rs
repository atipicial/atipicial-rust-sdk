...
use crate::{
	...
	atipicial_clients::JsonRpcProvider,
	atipicial_contract::{ContractError, AtipicialIterator, NftContract, TokenTrait},
...
pub trait NonFungibleTokenTrait<'a, P: JsonRpcProvider>: TokenTrait<'a, P> + Send {
	...
	async fn tokens_of(&mut self, owner: H160) -> Result<AtipicialIterator<Bytes, P>, ContractError> {
	...
	async fn transfer_to_name(
		...
		let name = NNSName::new(to).map_err(|e| ContractError::InvalidAtipicialName(e.to_string()))?;
	...
	async fn owners_of(
		&mut self,
		token_id: Bytes,
	) -> Result<AtipicialIterator<Address, P>, ContractError> {
		self.throw_if_non_divisible_nft().await?;
	...
	async fn tokens(&mut self) -> Result<AtipicialIterator<Bytes, P>, ContractError> {
		self.call_function_returning_iterator(
