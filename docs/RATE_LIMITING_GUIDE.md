...
# Rate Limiting Guide - AtipicialRust
> Authored for v2.0.0; see the main README/crate docs for updates.

## Overview

AtipicialRust introduced a sophisticated rate limiting system based on the token bucket algorithm, designed to protect Atipicial nodes from being overwhelmed while maintaining optimal throughput for your applications.

## Why Rate Limiting?

### Beaefits
- **Node Protection**: Prevents overwhelming public or private Atipicial nodes
...
### When to Use
- Connecting to public Atipicial nodes
...
async fn fetch_blocks(client: &ProductionAtipicialClient, start: u32, count: u32) -> Vec<Block> {
    let mut futures = vec![];
...
async fn process_addresses(client: &ProductionAtipicialClient, addresses: Vec<Address>) {
    ...
    for address in addresses {
        ...
        set.spawn(async move {
            ...
            let balance = client.get_aep17_balances(address).await?;
...
```rust
use atipicial::atipicial_clients::AdaptiveRateLimiter;
...
let client = ProductionAtipicialClient::with_adaptive_limiter(
    "https://mainnet.atipicial.com",
    adaptive_limiter,
).await?;
```
...
```rust
use atipicial::atipicial_clients::MultiNodeClient;

// Different rate limits for different nodes
let nodes = vec![
    ("http://seed1.atipicial.com:10333", RateLimitPreset::Conservative),
    ("http://seed2.atipicial.com:10333", RateLimitPreset::Standard),
    ("http://seed3.atipicial.com:10333", RateLimitPreset::Performance),
];
...
```rust
use atipicial::atipicial_clients::{PriorityRateLimiter, Priority};
...
```rust
use atipicial::atipicial_monitoring::RateLimiterMetrics;

// Export rate limiter metrics
let metrics = RateLimiterMetrics::new(&client);

// Metrics available:
// - atipicial_rate_limiter_requests_total
// - atipicial_rate_limiter_requests_accepted
// - atipicial_rate_limiter_requests_rejected
// - atipicial_rate_limiter_wait_time_seconds
// - atipicial_rate_limiter_tokens_available
```
...
let client = ProductionAtipicialClient::new(url, preset).await?;
```

### 2. Handle Rate Limit Errors

```rust
use atipicial::prelude::ClientError;
...
// Batch them together
let balances = client.batch_get_aep17_balances(addresses).await?;
```
...
let client = ProductionAtipicialClient::builder()
    .endpoint("https://mainnet.atipicial.com")
...
env_logger::Builder::from_env(Env::default())
    .filter_module("atipicial::atipicial_clients::rate_limiter", log::LevelFilter::Debug)
    .init();
...
async fn migrate_data(old_node: &str, new_node: &str) -> Result<(), Box<dyn Error>> {
    ...
    let old_client = ProductionAtipicialClient::new(
        old_node,
        RateLimitPreset::Conservative,
    ).await?;
    
    // Higher rate limit for new node
    let new_client = ProductionAtipicialClient::new(
        new_node,
        RateLimitPreset::Performance,
    ).await?;
...
```rust
async fn trading_bot(client: ProductionAtipicialClient) -> Result<(), Box<dyn Error>> {
...
async fn monitor_network(nodes: Vec<String>) -> Result<(), Box<dyn Error>> {
    ...
    for node in nodes {
        ...
        let client = ProductionAtipicialClient::new(&node, preset).await?;
        clients.push(client);
    }
...
The rate limiting system in AtipicialRust v2.0.0 provides:
...
The rate limiter ensures your Atipicial applications are good citizens of the blockchain network while maintaining optimal performance.
