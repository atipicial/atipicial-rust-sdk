//! Example demonstrating new features in AtipicialRust v1.0.6
//!
...
use atipicial::atipicial_error::unified::AtipicialError as UnifiedAtipicialError;
#[cfg(feature = "ws")]
use atipicial::sdk::websocket::{SubscriptionType, WebSocketClient};
use atipicial::sdk::{hd_wallet::*, transaction_simulator::*, Atipicial};
use std::sync::Arc;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	println!("🚀 AtipicialRust v1.0.6 Feature Demonstration\n");
	...
	#[cfg(feature = "ws")]
	{
		...
		println!("   Note: Requires a Atipicial node with WebSocket support");
	...
	#[cfg(not(feature = "ws"))]
	{
		...
		println!("   Enable with: atipicial = {{ version = \"1.0.6\", features = [\"ws\"] }}");
		println!();
	}
	...
	match atipicial.get_balance("invalid_address").await {
		Ok(_) => println!("   Unexpected success"),
		Err(e) => {
			println!("   Error: {}", e);

			let recovery = match &e {
				UnifiedAtipicialError::Network { recovery, .. }
				| UnifiedAtipicialError::Wallet { recovery, .. }
				| UnifiedAtipicialError::Contract { recovery, .. }
				| UnifiedAtipicialError::Transaction { recovery, .. }
				| UnifiedAtipicialError::Configuration { recovery, .. }
				| UnifiedAtipicialError::Validation { recovery, .. }
				| UnifiedAtipicialError::InsufficientFunds { recovery, .. }
				| UnifiedAtipicialError::Timeout { recovery, .. }
				| UnifiedAtipicialError::RateLimit { recovery, .. }
				| UnifiedAtipicialError::Other { recovery, .. } => Some(recovery),
				_ => None,
			};
	...
	// ========================================
	println!("✨ Summary");
	println!("   AtipicialRust v1.0.6 provides:");
	...
	println!("   ✅ Interactive CLI wizard (atipicial-cli wizard)");
