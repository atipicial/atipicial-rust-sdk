...
use crate::{
	atipicial_builder::TransactionBuilder,
...
impl<'a, P: JsonRpcProvider + 'static> AtipicialCoin<'a, P> {
	...
	pub async fn get_account_state(&self, account: &H160) -> Result<AccountState, ContractError> {
		...
		match result {
			...
			StackItem::Array { value: items } if items.len() >= 3 => {
				...
				// Atipicial 3.9+ includes last_gas_per_vote as the 4th element
			...
			_ => Err(ContractError::InvalidAtipicialName("Account state malformed".to_string())),
		}
	}
	...
	async fn resolve_nns_text_record(&self, _name: &NNSName) -> Result<H160, ContractError> {
		// ATC token doesn't support NNS text record resolution
		// Return an error indicating this operation is not supported
		Err(ContractError::UnsupportedOperation(
			"NNS text record resolution is not supported for ATC token".to_string(),
		))
	}
}

#[async_trait]
impl<'a, P: JsonRpcProvider> TokenTrait<'a, P> for AtipicialCoin<'a, P> {
...
#[async_trait]
impl<'a, P: JsonRpcProvider> SmartContractTrait<'a> for AtipicialCoin<'a, P> {
	type P = P;
...
#[async_trait]
impl<'a, P: JsonRpcProvider> FungibleTokenTrait<'a, P> for AtipicialCoin<'a, P> {}
...
/// Represents the account state for ATC token holders.
///
/// The account state tracks the ATC balance, the block height when the balance was last updated,
/// the voting target (if any), and the last gas per vote value for reward calculations.
pub struct AccountState {
	/// The ATC balance of the account.
	pub balance: i64,
	...
	/// This field was added in Atipicial 3.9 for improved voting reward tracking.
