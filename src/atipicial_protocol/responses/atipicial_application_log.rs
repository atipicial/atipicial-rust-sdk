...
use crate::{atipicial_protocol::LogNotification, TypeError};
use atipicial::prelude::{deserialize_h256, serialize_h256, StackItem, VMState};
...
impl Execution {
	pub fn get_first_stack_item(&self) -> Result<&StackItem, TypeError> {
		if self.stack.is_empty() {
			return Err(TypeError::IndexOutOfBounds(
                "The stack is empty. This means that no items were left on the AtipicialVM stack after this execution."
                    .to_string(),
            ));
		}
	...
	pub fn get_stack_item(&self, index: usize) -> Result<&StackItem, TypeError> {
		if index >= self.stack.len() {
			return Err(TypeError::IndexOutOfBounds(format!(
				"There were only {} items left on the AtipicialVM stack after this execution.",
				self.stack.len()
			)));
		}
		Ok(&self.stack[index])
