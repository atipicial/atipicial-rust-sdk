...
# AtipicialRust SDK - API Specification
...
```rust
use atipicial::atipicial_builder::{TransactionBuilder, ScriptBuilder, Signer};
...
use atipicial::atipicial_wallets::{Wallet, WalletTrait};
...
```rust
use atipicial::atipicial_contract::{SmartContract, ContractManifest};
...
```rust
// Load contract files
let aef = AefFile::from_file("contract.aef")?;
...
### AEP-17 Token Operations

```rust
use atipicial::atipicial_contract::Aep17Token;

// Initialize AEP-17 token
let atipicial = Aep17Token::new(&client, ATC_TOKEN_HASH);
let gas = Aep17Token::new(&client, GAS_TOKEN_HASH);

// Query token info
let symbol = atipicial.symbol().await?;
let decimals = atipicial.decimals().await?;
let total_supply = atipicial.total_supply().await?;

// Check balance
let balance = atipicial.balance_of(&account_hash).await?;

// Transfer tokens
let tx = atipicial.transfer(
    &from_account,
    &to_address,
    amount,
...
```rust
use atipicial::atipicial_clients::SubscriptionStream;
...
```rust
use atipicial::atipicial_crypto::{sha256, hash160, hash256};
...
```rust
use atipicial::atipicial_crypto::Aep2;

// Encrypt private key
let encrypted = Aep2::encrypt(&private_key, "password")?;
println!("Encrypted: {}", encrypted);

// Decrypt private key
let decrypted = Aep2::decrypt(&encrypted, "password")?;
```
...
```rust
use atipicial::atipicial_error::{AtipicialError, AtipicialResult};

// Comprehensive error handling
match client.get_block_count().await {
    Ok(count) => println!("Block count: {}", count),
    Err(AtipicialError::Network(e)) => eprintln!("Network error: {}", e),
    Err(AtipicialError::Crypto(e)) => eprintln!("Crypto error: {}", e),
    Err(AtipicialError::Transaction(e)) => eprintln!("Transaction error: {}", e),
...
fn validate_amount(amount: u64) -> AtipicialResult<()> {
...
```rust
use atipicial::atipicial_clients::{RateLimiter, RateLimiterBuilder, RateLimiterPresets};
...
let ws = Ws::connect("wss://mainnet1.atipicial.com/ws").await?;
