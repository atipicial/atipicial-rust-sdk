#![deny(missing_docs)]
//! High-level, AWS-style entrypoint for Atipicial.
//!
//! The `sdk` module is the recommended starting point for application code.
//! It exposes a small, opinionated surface ([`Atipicial`], [`AtipicialBuilder`],
...
//!
//! // Or honour `ATC_RPC_URL` from the environment for 12-factor deployments.
...
//! # #[tokio::main]
//! # async fn main() -> Result<(), Box<dyn std::error::Error>> {
//! let atipicial = Atipicial::builder()
...
//! [`crate::atipicial_error::unified::AtipicialError`] type. It supports
//! AWS-SDK-style classification:
//!
//! ```no_run
//! use atipicial::sdk::Atipicial;
//! use atipicial::atipicial_error::unified::AtipicialErrorKind;
//!
//! # #[tokio::main]
//! # async fn main() {
//! let atipicial = Atipicial::testnet().await.unwrap();
//! if let Err(err) = atipicial.get_block_height().await {
//!     if err.is_retryable() {
...
//!     }
//!     match err.kind() {
//!         AtipicialErrorKind::RateLimit => { /* back off */ }
//!         AtipicialErrorKind::Network   => { /* health-check endpoint */ }
//!         _ => {}
//!     }
//! }
//! # }
...
//!   bridging Atipicial ↔ Atipicial X (EVM).
//! - `websocket` — push-based event subscriptions (requires `ws` feature).

pub mod hd_wallet;
mod retry;
pub mod transaction_simulator;
/// Cross-chain unified client bridging Atipicial and Atipicial X (EVM).
///
...
use crate::{
	atipicial_clients::{APITrait, HttpProvider, RpcCache, RpcClient},
	atipicial_error::unified::{ErrorRecovery, AtipicialError},
	atipicial_types::VMState,
	atipicial_types::{ContractParameter, ScriptHash, ScriptHashExtension, StackItem},
	atipicial_wallets::wallet::Wallet,
};
use hex_literal::hex;
...
/// Main entry point for the Atipicial SDK
///
...
///
/// Atipicial AEP-17 balances are returned as raw integers (base units). This type keeps the raw
...
impl DecimalAmount {
	...
	/// Convert raw base-units to `i64` when it fits (useful for Atipicial VM integers).
...
impl Token {
	/// Native ATC governance token script hash.
	pub const ATC_HASH: [u8; 20] = hex!("ef4073a0f2b305a38ec4050e4d3d28bc40ea63f5");
	...
	pub fn contract_hash(&self) -> ScriptHash {
		match self {
			Token::ATC => ScriptHash::from(Self::ATC_HASH),
...
pub(crate) fn no_default_account_error() -> AtipicialError {
...
fn invalid_address_error<E: fmt::Display>(field: &str, value: &str, err: E) -> AtipicialError {
	AtipicialError::Validation {
		...
		recovery: ErrorRecovery::new()
			...
			.suggest("Ensure it's a valid Atipicial address"),
	}
}
...
async fn send_tx_with_retry<'a>(
	tx: &mut crate::atipicial_builder::Transaction<'a, HttpProvider>,
	attempts: u32,
	delay: Duration,
	context: &str,
) -> Result<crate::atipicial_protocol::RawTransaction, AtipicialError> {
	let attempts = attempts.max(1);
	let tx_id = tx.tx_id().map_err(|err| AtipicialError::transaction(context, err))?;
	for attempt in 1..=attempts {
		match tx.send_tx().await {
			...
			Err(err) => {
				if matches!(
					&err,
					crate::atipicial_builder::TransactionError::ProviderError(error)
						if error.is_already_known_transaction()
				) {
					tracing::debug!(transaction_hash = %tx_id, "transaction already accepted by node");
					return Ok(crate::atipicial_protocol::RawTransaction::new(tx_id));
				}
				...
				if !retryable || attempt == attempts {
					return Err(match err {
						...
						crate::atipicial_builder::TransactionError::ProviderError(error) => {
							AtipicialError::provider(context, error)
						},
						other => AtipicialError::transaction(context, other),
					});
				}
...
/// Build, sign, and send a AEP-17 transfer with retry-aware broadcast.
///
/// Shared by [`Atipicial::transfer`] and [`Transfer::execute`] so both paths get
...
async fn build_and_send_transfer(
	...
	from_account: &crate::atipicial_protocol::Account,
	to: &str,
	amount: u64,
	token: Token,
	memo: Option<&str>,
	max_attempts: u32,
) -> Result<TxHash, AtipicialError> {
	use crate::atipicial_builder::{AccountSigner, CallFlags, ScriptBuilder, TransactionBuilder};
	use crate::atipicial_types::ScriptHashExtension;

	let to_hash = ScriptHash::from_address(to).map_err(|e| invalid_address_error("to", to, e))?;
	let contract_hash = token.contract_hash();

	let amount_i64 = i64::try_from(amount).map_err(|_| AtipicialError::Validation {
		message: "Amount is too large to fit Atipicial VM integer".to_string(),
	...
	// The 4th transfer argument is the AEP-17 `data` parameter: forward a
	...
	let signer = AccountSigner::called_by_entry(from_account)
		.map_err(|e| AtipicialError::transaction("Failed to create signer", e))?;
	...
	tb.set_signers(vec![signer.into()])
		.map_err(|e| AtipicialError::transaction("Failed to set signers", e))?;
	tb.valid_until_block(current_height + 5760)
		.map_err(|e| AtipicialError::transaction("Invalid valid-until-block", e))?;

	let mut tx = tb
		.sign()
		.await
		.map_err(|e| AtipicialError::transaction("Failed to sign transfer", e))?;
...
impl Atipicial {
	/// Connect to Atipicial TestNet with default configuration
	///
	/// # Examples
	///
	/// ```no_run
	...
	/// # }
	/// ```
	pub async fn testnet() -> Result<Self, AtipicialError> {
		Self::builder().network(Network::TestNet).build().await
	}

	/// Connect to Atipicial MainNet with default configuration
	///
	/// # Examples
	///
	/// ```no_run
	...
	/// # #[tokio::main]
	/// # async fn main() -> Result<(), Box<dyn std::error::Error>> {
	/// let atipicial = Atipicial::mainnet().await?;
	/// # Ok(())
	/// # }
	/// ```
	pub async fn mainnet() -> Result<Self, AtipicialError> {
		Self::builder().network(Network::MainNet).build().await
	}

	/// Connect to a custom RPC endpoint with default configuration.
	///
	/// Convenience for `Atipicial::builder().network(Network::Custom(...)).build()`,
	...
	/// # }
	/// ```
	pub async fn connect(endpoint: impl Into<String>) -> Result<Self, AtipicialError> {
		Self::builder().network(Network::Custom(endpoint.into())).build().await
	}

	/// Build a [`Atipicial`] using `$ATC_RPC_URL` from the environment, falling back to TestNet.
	///
	...
	/// | Env var | Effect |
	/// |---------|--------|
	/// | `ATC_RPC_URL` | Connect to this RPC endpoint (overrides everything). |
	...
	/// # #[tokio::main]
	/// # async fn main() -> Result<(), Box<dyn std::error::Error>> {
	/// // export ATC_RPC_URL=https://mainnet1.atipicial.com:443
	/// let atipicial = Atipicial::from_env().await?;
	/// # Ok(())
	/// # }
	/// ```
	pub async fn from_env() -> Result<Self, AtipicialError> {
		match std::env::var("ATC_RPC_URL") {
	...
	pub fn builder() -> AtipicialBuilder {
		AtipicialBuilder::default()
	}

	/// Get the balance of an address
	///
	/// Returns ATC, GAS, and all AEP-17 token balances.
	///
	/// # Examples
	///
	/// ```no_run
	...
	/// let atipicial = Atipicial::testnet().await?;
	/// let balance = atipicial.get_balance("NbTiM6h8r99kpRtb428XcsUk1TzKed2gTc").await?;
	/// println!("ATC: {}, GAS: {}", balance.atipicial, balance.gas);
	/// # Ok(())
	/// # }
	/// ```
	pub async fn get_balance(&self, address: &str) -> Result<Balance, AtipicialError> {
	...
	async fn get_balance_inner(&self, address: &str) -> Result<Balance, AtipicialError> {
		use crate::atipicial_types::ScriptHashExtension;
		...
		let atipicial_hash = ScriptHash::from(Token::ATC_HASH);
		let atipicial_balance =
			retry_network("fetch ATC balance", max_attempts, DEFAULT_RETRY_DELAY, || async {
				self.client
					.invoke_function(
						&atipicial_hash,
						"balanceOf".to_string(),
		...
		let atipicial = atipicial_balance
			.stack
			.first()
			.ok_or_else(|| invalid_balance_response("ATC", "missing stack item"))?;
		let atipicial = parse_balance_stack_item_u64(atipicial, "ATC")?;
		...
		let aep17 =
			retry_network("fetch AEP-17 balances", max_attempts, DEFAULT_RETRY_DELAY, || async {
				self.client.get_aep17_balances(script_hash).await
			})
			.await?;

		let mut tokens = Vec::new();
		for b in aep17.balances {
			if b.asset_hash == atipicial_hash || b.asset_hash == gas_hash {
				continue;
			}

			// Standard `getaep17balances` responses omit `symbol`/`decimals`;
			...
			let decimals = match b.decimals.as_deref() {
				Some(raw) => parse_aep17_decimals(raw, &asset_hash)?,
				None => self.fetch_aep17_decimals(&asset_hash).await?,
			};
		...
		let balance = Balance { atipicial, gas, tokens };
	...
	/// Query a AEP-17 token's `decimals()` from the token contract itself.
	///
	/// Used when the node's `getaep17balances` response omits token metadata.
	async fn fetch_aep17_decimals(&self, contract: &ScriptHash) -> Result<u8, AtipicialError> {
		let result = self
			.client
	...
	/// Query a AEP-17 token's `symbol()` from the token contract itself.
	async fn fetch_aep17_symbol(&self, contract: &ScriptHash) -> Option<String> {
		let result = self
			.client
	...
	/// use atipicial::sdk::{Atipicial, Token};
	///
	/// # #[tokio::main]
	/// # async fn main() -> Result<(), Box<dyn std::error::Error>> {
	/// let atipicial = Atipicial::testnet().await?;
	/// let wallet = Wallet::new();
	/// let tx_hash = atipicial
	///     .transfer(
	///         &wallet,
	...
	/// let atipicial = Atipicial::testnet().await?;
	/// let wallet = Wallet::new();
	/// let aef_bytes = Vec::new();
	/// let manifest = "{}".to_string();
	///
	/// let contract_hash = atipicial.deploy_contract(&wallet, aef_bytes, manifest).await?;
	...
	pub async fn deploy_contract(
		...
		aef: Vec<u8>,
		manifest: String,
	) -> Result<ScriptHash, AtipicialError> {
		let started = std::time::Instant::now();
		let result = self.deploy_contract_inner(deployer, aef, manifest).await;
	...
	async fn deploy_contract_inner(
		...
		use crate::atipicial_builder::{AccountSigner, CallFlags, ScriptBuilder, TransactionBuilder};
		use crate::atipicial_types::{ContractManifest, AefFile, ScriptHashExtension};
		use crate::atipicial_wallets::WalletTrait;
		...
		let aef_file = AefFile::deserialize(&aef).map_err(|e| AtipicialError::Validation {
			message: format!("Invalid AEF file: {}", e),
			field: "aef".to_string(),
			value: None,
			recovery: ErrorRecovery::new()
				.suggest("Ensure the AEF bytes are valid")
				.suggest("Load the AEF file using std::fs::read"),
		})?;

		let manifest_bytes = manifest.as_bytes().to_vec();
		let manifest_struct: ContractManifest =
			serde_json::from_str(&manifest).map_err(|e| AtipicialError::Validation {
		...
		let contract_name = manifest_struct.name.clone().ok_or_else(|| AtipicialError::Validation {
		...
		let aef_checksum = {
			if aef_file.checksum.len() != 4 {
				return Err(AtipicialError::Validation {
					message: "AEF checksum length is invalid".to_string(),
					field: "aef.checksum".to_string(),
					value: None,
					recovery: ErrorRecovery::new().suggest("Provide a valid AEF file"),
				});
			}
			let mut arr = [0u8; 4];
			arr.copy_from_slice(&aef_file.checksum);
			arr.reverse();
		...
		.map_err(|e| {
			AtipicialError::contract("Failed to derive contract script", None, Some("deploy".into()), e)
		})?;
		...
		sb.contract_call(
			...
			"deploy",
			&[
				ContractParameter::byte_array(aef),
		...
		tb.extend_script(sb.to_bytes());
		tb.set_signers(vec![signer.into()])
			.map_err(|e| AtipicialError::transaction("Failed to set signer", e))?;
		...
		let mut tx = tb
			.sign()
			.await
			.map_err(|e| AtipicialError::transaction("Failed to sign deploy transaction", e))?;
	...
	pub async fn invoke_read(
	...
	) -> Result<serde_json::Value, AtipicialError> {
		use crate::atipicial_types::ScriptHashExtension;
		...
		if result.has_state_fault() {
			return Err(AtipicialError::Contract {
				message: result
					.exception
					.clone()
		...
		serde_json::to_value(result).map_err(|e| AtipicialError::Other {
	...
	async fn invoke_write_inner(
		...
		let mut tx = tb
			.sign()
			.await
			.map_err(|e| AtipicialError::transaction("Failed to sign invocation transaction", e))?;
	...
	/// atipicial.wait_for_confirmation(tx_hash, Duration::from_secs(60)).await?;
	/// # Ok(())
	/// # }
	/// ```
	pub async fn wait_for_confirmation(
		...
		tx_hash: &str,
		timeout: Duration,
	) -> Result<(), AtipicialError> {
		use primitive_types::H256;
		use std::str::FromStr;
		use std::time::Instant;

		let tx_h256 = H256::from_str(tx_hash).map_err(|e| AtipicialError::Validation {
		...
		while start.elapsed() < timeout {
			match self.client.get_application_log(tx_h256).await {
				Ok(log) => {
					...
					{
						return Err(AtipicialError::Transaction {
							message: format!(
				...
				Err(err) => return Err(AtipicialError::provider("poll transaction confirmation", err)),
			}
		}

		Err(AtipicialError::Timeout {
			duration: timeout,
	...
	pub async fn get_block_height(&self) -> Result<u32, AtipicialError> {
		let started = std::time::Instant::now();
	...
	async fn get_block_height_inner(&self) -> Result<u32, AtipicialError> {
		let max_attempts = self.config.retries.saturating_add(1);
...
/// Builder for configuring the Atipicial SDK
#[derive(Debug)]
...
impl Default for AtipicialBuilder {
	fn default() -> Self {
		Self { network: Network::TestNet, config: SdkConfig::default() }
	}
}

impl AtipicialBuilder {
	...
	/// Build the Atipicial SDK instance
	pub async fn build(self) -> Result<Atipicial, AtipicialError> {
		let endpoint = match &self.network {
			Network::MainNet => "https://mainnet1.atipicial.com:443".to_string(),
			Network::TestNet => "https://testnet1.atipicial.coz.io:443".to_string(),
			Network::Custom(url) => url.clone(),
		};

		let url = url::Url::parse(&endpoint).map_err(|e| AtipicialError::Network {
		...
		let max_attempts = self.config.retries.saturating_add(1);
		retry_network("connect to Atipicial network", max_attempts, DEFAULT_RETRY_DELAY, || async {
			client.get_block_count().await
		})
		.await?;
		...
		Ok(Atipicial { client, network: self.network, endpoint, cache, config: self.config })
	}
}
...
impl Transfer {
	...
	pub async fn execute(self, client: &RpcClient<HttpProvider>) -> Result<TxHash, AtipicialError> {
		use crate::atipicial_wallets::WalletTrait;
		...
		build_and_send_transfer(
			...
			// attempt still beaefits from retry-aware height fetch and
...
fn invalid_balance_response(token: &str, detail: impl Into<String>) -> AtipicialError {
...
fn parse_balance_stack_item_u64(item: &StackItem, token: &str) -> Result<u64, AtipicialError> {
...
fn parse_aep17_decimals(decimals: &str, asset_hash: &ScriptHash) -> Result<u8, AtipicialError> {
	decimals.parse::<u8>().map_err(|_| AtipicialError::Other {
		message: format!(
			"Invalid decimals '{}' for AEP-17 token {}",
			decimals,
			asset_hash.to_hex()
		),
		source: None,
...
mod tests {
	...
	async fn transaction_with_broadcast_responses(
		...
		attempts: u32,
	) -> (Result<crate::atipicial_protocol::RawTransaction, AtipicialError>, primitive_types::H256) {
		...
		let mut transaction = crate::atipicial_builder::Transaction::<HttpProvider> {
			network: Some(&client),
			..Default::default()
		};
	...
	async fn send_retry_classifies_node_rejection_as_transaction_error() {
		...
		match result.unwrap_err() {
			AtipicialError::Transaction { tx_hash, recovery, .. } => {
	...
	fn endpoint_shortcut_picks_custom_network() {
		let builder = Atipicial::builder().endpoint("https://example.com:443");
	...
	fn config_setter_replaces_entire_config() {
		let cfg = SdkConfig::builder()
			...
			.build();
		let builder = Atipicial::builder().config(cfg.clone()).network(Network::TestNet);
	...
	fn token_contract_hash_returns_native_hashes() {
		let atipicial = Token::ATC.contract_hash();
		let gas = Token::GAS.contract_hash();
		assert_eq!(<[u8; 20]>::from(atipicial), Token::ATC_HASH);
	...
	async fn connect_with_malformed_url_returns_network_error() {
		...
		let err = Atipicial::connect("not-a-url").await.unwrap_err();
		assert_eq!(err.kind(), crate::atipicial_error::unified::AtipicialErrorKind::Network);
	...
	async fn from_env_without_env_var_falls_back_to_testnet() {
		...
		let result = Atipicial::from_env().await;

		// Restore env to whatever it was.
		if let Some(value) = prior {
			std::env::set_var("ATC_RPC_URL", value);
		}

		if let Err(err) = result {
			let kind = err.kind();
			assert!(
				matches!(kind, crate::atipicial_error::unified::AtipicialErrorKind::Network),
	...
	async fn from_env_with_malformed_url_yields_network_error() {
		let prior = std::env::var_os("ATC_RPC_URL");
		std::env::set_var("ATC_RPC_URL", "::::not a url::::");

		let result = Atipicial::from_env().await;

		// Restore env first.
		match prior {
			Some(value) => std::env::set_var("ATC_RPC_URL", value),
			None => std::env::remove_var("ATC_RPC_URL"),
		}

		let err = result.expect_err("malformed ATC_RPC_URL must error");
		assert_eq!(err.kind(), crate::atipicial_error::unified::AtipicialErrorKind::Network);
	}
	...
	fn test_parse_balance_stack_item_u64_rejects_negative_value() {
		...
		let err = parse_balance_stack_item_u64(&item, "ATC").unwrap_err();
	...
	#[test]
	fn test_parse_aep17_decimals_rejects_invalid_value() {
		let asset_hash = ScriptHash::zero();
		let err = parse_aep17_decimals("not-a-u8", &asset_hash).unwrap_err();
