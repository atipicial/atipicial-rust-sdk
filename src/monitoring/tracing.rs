...
macro_rules! trace_transaction {
	($tx_type:expr, $network:expr, $body:block) => {{
		let span = tracing::info_span!("atipicial.transaction", tx_type = $tx_type, network = $network);
...
macro_rules! trace_rpc {
	($method:expr, $endpoint:expr, $body:block) => {{
		let span = tracing::info_span!("atipicial.rpc", method = $method, endpoint = $endpoint);
		let _guard = span.enter();
		$body
	}};
}
...
macro_rules! trace_contract {
	($contract:expr, $operation:expr, $body:block) => {{
		let span =
			tracing::info_span!("atipicial.contract", contract = $contract, operation = $operation);
		let _guard = span.enter();
		$body
	}};
}
