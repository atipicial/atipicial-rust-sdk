...
use crate::{
	atipicial_builder::{CallFlags, ScriptBuilder},
	atipicial_clients::{APITrait, JsonRpcProvider, RpcClient},
	atipicial_contract::{ContractError, AtipicialIterator},
	atipicial_types::{
...
use crate::atipicial_builder::{Signer, TransactionBuilder};
...
pub trait SmartContractTrait<'a>: Send + Sync {
	...
	async fn build_invoke_function_script(
		...
		if function.is_empty() {
			return Err(ContractError::InvalidAtipicialName("Function name cannot be empty".to_string()));
		}
	...
	async fn call_invoke_function(
		...
		if function.is_empty() {
			return Err(ContractError::InvalidAtipicialName("Function cannot be empty".to_string()));
		}
	...
	) -> Result<AtipicialIterator<U, Self::P>, ContractError>
	where
		U: Send + Sync, // Adding this bound if necessary
	{
		...
		Ok(AtipicialIterator::new(session_id, id.clone(), mapper, Some(provider)))
	}
	...
	///
	/// This is intended for use with known-good constants like `AtipicialCoin`, `GasToken`, etc.,
	...
	fn calc_contract_hash(
		...
		if contract_name.is_empty() {
			return Err(ContractError::InvalidAtipicialName("Contract name cannot be empty".to_string()));
		}
...
mod tests {
	use super::*;
	use crate::{
		atipicial_clients::{MockProvider, RpcClient},
		atipicial_contract::ContractError,
		atipicial_types::{ContractManifest, ContractAef, ContractState},
	};
	use async_trait::async_trait;
	use primitive_types::H160;
	use serde_json::json;
	...
	fn test_contract_state(hash: H160, manifest: ContractManifest) -> ContractState {
		ContractState::new(1, 0, hash, ContractAef::default(), manifest)
	}
