...
impl Default for AtipicialVersion {
	fn default() -> Self {
		AtipicialVersion {
			tcp_port: Some(10333),
			ws_port: Some(10334),
			nonce: 1234567890,
			user_agent: "/Atipicial:3.5.0/".to_string(),
			rpc: None,
			protocol: Some(AtipicialProtocol::default()),
		}
	}
}
...
fn default_user_agent() -> String {
	"/Atipicial:3.5.0/".to_string()
}

fn default_protocol() -> Option<AtipicialProtocol> {
	Some(AtipicialProtocol::default())
}

impl PartialEq for AtipicialVersion {
...
impl Default for AtipicialProtocol {
	fn default() -> Self {
		AtipicialProtocol {
			network: 860833102,
