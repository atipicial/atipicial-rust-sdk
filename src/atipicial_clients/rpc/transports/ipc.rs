...
use crate::atipicial_clients::{JsonRpcProvider, ProviderError, PubsubClient, RpcError};
...
/// use atipicial::atipicial_clients::Ipc;
///
/// // the ipc's path
#[cfg_attr(unix, doc = r#"let path = "/tmp/atipicial-node.ipc";"#)]
#[cfg_attr(windows, doc = r#"let path = r"\\.\pipe\atipicial-node.ipc";"#)]
...
impl Shared {
	...
	async fn handle_ipc_reads(&self, reader: ReadHalf<'_>) -> Result<Infallible, IpcError> {
		...
		let max_buffer_size: usize = AtipicialConstants::max_rpc_message_size();
	...
	async fn handle_ipc_writes(
		...
		let request_timeout = AtipicialConstants::rpc_request_timeout();
...
mod tests {
	...
	use super::Ipc;
	use crate::{
		atipicial_types::{block::Block, TxHash},
		prelude::U256,
	};
	...
	async fn request() {
		...
		let block_num: U256 = ipc.fetch("atipicial_blockNumber", ()).await.unwrap();
		tokio::time::sleep(Duration::from_secs(2)).await;
		let block_num2: U256 = ipc.fetch("atipicial_blockNumber", ()).await.unwrap();
	...
	async fn subscription() {
		...
		// the returned sub_id
		let sub_id: U256 = ipc.fetch("atipicial_subscribe", ["newHeads"]).await.unwrap();
