...
impl PooledConnection {
	fn new(endpoint: &str) -> AtipicialResult<Self> {
		let provider = HttpProvider::new(endpoint).map_err(|e| {
			AtipicialError::Network(crate::atipicial_error::NetworkError::ConnectionFailed(e.to_string()))
		})?;
...
/// High-performance connection pool for Atipicial RPC clients
...
impl ConnectionPool {
	...
	pub async fn execute<F, T>(&self, operation: F) -> AtipicialResult<T>
	where
		F: Fn(
				&RpcClient<HttpProvider>,
			)
				-> std::pin::Pin<Box<dyn std::future::Future<Output = AtipicialResult<T>> + Send + '_>>
			+ Send
			+ Sync,
		T: Send,
	{
		let _permit = self.semaphore.acquire().await.map_err(|_| {
			AtipicialError::Network(crate::atipicial_error::NetworkError::ConnectionFailed(
			...
			match result {
				...
				Err(_) => {
					...
					if retries < self.config.max_retries {
						...
						return Err(AtipicialError::Network(crate::atipicial_error::NetworkError::Timeout));
					}
				},
			}
		}
	...
	async fn get_connection(&self) -> AtipicialResult<PooledConnection> {
...
mod tests {
	use super::*;
	use crate::atipicial_protocol::AtipicialVersion;

	#[tokio::test]
	async fn test_pool_creation() {
		let config = PoolConfig::default();
		let pool = ConnectionPool::new("https://testnet.atipicial.com:443".to_string(), config);
	...
	async fn test_pool_stats() {
		...
		let _result =
			pool.execute(|_client| Box::pin(async move { Ok(AtipicialVersion::default()) })).await;
