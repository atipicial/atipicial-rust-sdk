/// A builder for constructing and configuring ATC blockchain transactions.
///
...
/// #[tokio::main]
/// async fn main() {
///     let provider = HttpProvider::new("https://testnet1.atipicial.com:443").unwrap();
...
// Import from atipicial_types
use crate::atipicial_types::{Bytes, ContractParameter, InvocationResult, ScriptHash};

// Import transaction types from atipicial_builder
use crate::atipicial_builder::{
	transaction::{
...
use crate::{
	...
	atipicial_config::{AtipicialConstants, ATCCONFIG},
	atipicial_crypto::{utils::ToHexString, Secp256r1PublicKey},
	atipicial_protocol::AccountTrait,
};

// Helper functions
use crate::atipicial_clients::public_key_to_script_hash;

// Import Account from atipicial_protocol
use crate::atipicial_protocol::Account;
...
impl<'a, P: JsonRpcProvider + 'static> TransactionBuilder<'a, P> {
	...
	/// ```rust
	/// use atipicial::atipicial_builder::TransactionBuilder;
	/// use atipicial::atipicial_clients::HttpProvider;
	///
	...
	///
	/// This overrides any global `ATCCONFIG` setting for this builder instance.
	...
	/// # Arguments
	///
	/// * `version` - The transaction version (typically 0 for Atipicial).
	///
	/// # Returns
	///
	...
	///
	/// In Atipicial, transactions have a limited validity period defined by block height.
	...
	pub fn validate(&self) -> Result<(), TransactionError> {
		...
		// Check signer limits
		if self.signers.len() > AtipicialConstants::MAX_SIGNER_SUBITEMS as usize {
	...
	async fn get_system_fee(&self) -> Result<i64, TransactionError> {
		...
		if response.has_state_fault() {
			...
			//
			// Prefer a per-builder override when set; otherwise fall back to the global `ATCCONFIG`.
			let allows_fault = match self.allows_transmission_on_fault {
				...
				None => {
					ATCCONFIG
						.lock()
						.map_err(|_| {
							TransactionError::IllegalState("Failed to lock ATCCONFIG".to_string())
						})?
	...
	///
	/// ```no_run
	/// use atipicial::atipicial_builder::{TransactionBuilder, ScriptBuilder, AccountSigner};
	/// use atipicial::atipicial_clients::{HttpProvider, RpcClient};
	/// use atipicial::atipicial_protocol::{Account, AccountTrait};
	/// use atipicial::atipicial_types::ContractParameter;
	/// use std::str::FromStr;
	/// use atipicial::atipicial_clients::APITrait;
	///
	/// #[tokio::main]
	...
	///     let contract_hash = atipicial::atipicial_types::ScriptHash::from_str("0xd2a4cff31913016155e38e474a2c06d08be276cf")?; // GAS
	///     let from_address = account.get_script_hash();
	///     let to_address = atipicial::atipicial_types::ScriptHash::from_str("0x0000000000000000000000000000000000000000")?;
	...
	///
	/// ```rust
	/// use atipicial::atipicial_builder::{TransactionBuilder, Signer, AccountSigner};
	/// use atipicial::atipicial_protocol::{Account, AccountTrait};
	///
	...
	///
	/// ```rust
	/// use atipicial::atipicial_builder::{TransactionBuilder, TransactionAttribute};
	/// use atipicial::atipicial_clients::{HttpProvider, RpcClient};
	///
	/// let provider = HttpProvider::new("https://testnet1.atipicial.com:443").unwrap();
	/// let client = RpcClient::new(provider);
	...
	fn check_and_throw_if_max_attributes_exceeded(
		...
		let max_attributes = AtipicialConstants::MAX_TRANSACTION_ATTRIBUTES.try_into().map_err(|e| {
			TransactionError::IllegalState(format!(
		...
		if total_signers + total_attributes > max_attributes {
			return Err(TransactionError::TransactionConfiguration(format!(
				...
				AtipicialConstants::MAX_TRANSACTION_ATTRIBUTES
			)));
		}
		Ok(())
	}
