...
use atipicial::{
	...
	atipicial_clients::{APITrait, HttpProvider},
	atipicial_protocol::{Account, AccountTrait},
	atipicial_types::{AddressExtension, ContractParameter, ScriptHashExtension},
...
pub enum NftCommands {
	...
	/// Transfer a non-divisible AEP-11 token
	#[command(about = "Transfer a non-divisible AEP-11 token")]
	Transfer {
...
async fn send_nft_transaction(
	...
	if simulation.state != AtipicialVMStateType::Halt {
...
async fn handle_list_nfts(
	...
	let balances = rpc_client
		.get_aep11_balances(owner_hash)
		.await
		.map_err(|e| CliError::Network(format!("Failed to get AEP-11 balances: {e}")))?;
...
async fn handle_nft_info(
	...
	let properties = rpc_client
		.get_aep11_properties(contract_hash, &token_id)
		.await
