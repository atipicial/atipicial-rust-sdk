use atipicial::atipicial_fs::{
	client::{AtipicialFsAuth, AtipicialFsClient, AtipicialFsConfig, DEFAULT_TESTNET_REST_API},
...
/// Demonstrates building a multipart upload plan for AtipicialFs and simulating part assembly.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	println!("📦 AtipicialFs Multipart Upload Demo (local plan + optional API probe)");
	...
	let mut config_builder =
		AtipicialFsConfig::builder().endpoint(endpoint).timeout_sec(10).insecure(false);
	...
	println!("\n🔎 AtipicialFs API probe (initiate_multipart_upload)...");
