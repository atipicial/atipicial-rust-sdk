...
use crate::{
	...
	codec::{Decoder, Encoder, AtipicialSerializable, VarSizeTrait},
	config::AtipicialConstants,
	crypto::HashableForVec,
	atipicial_clients::{APITrait, HttpProvider, JsonRpcProvider, RpcClient},
	atipicial_protocol::{ApplicationLog, RawTransaction},
	atipicial_types::NameOrAddress,
	Bytes,
};

/// A Atipicial blockchain transaction.
///
/// The `Transaction` struct represents a transaction on the Atipicial blockchain. It contains
...
///
/// ```rust,no_run
/// use atipicial::atipicial_builder::{Transaction, TransactionBuilder};
...
impl<'de, 'a, P: JsonRpcProvider + 'static> Deserialize<'de> for Transaction<'a, P> {
		...
		D: Deserializer<'de>,
	{
		...
		// Atipicial RPC returns fees as strings (e.g. "9007810"), not JSON integers
		let sys_fee = value
			.get("sysfee")
...
impl<'a, T: JsonRpcProvider + 'static> Transaction<'a, T> {
	...
	/// Returns the transaction identifier (hash) as used by Atipicial nodes.
	///
	...
	/// Sends the transaction to the Atipicial network.
	///
	...
	///
	/// ```rust,no_run
	/// use atipicial::atipicial_builder::{ScriptBuilder, AccountSigner, TransactionBuilder, CallFlags};
	/// use atipicial::atipicial_clients::{HttpProvider, RpcClient, APITrait};
	/// use atipicial::atipicial_protocol::{Account, AccountTrait};
	/// use atipicial::atipicial_types::{Address, ContractParameter, ScriptHash, AddressExtension};
	/// use std::str::FromStr;
	///
	/// #[tokio::main]
...
// where
	// 	P: APITrait,
	{
		...
		if self.size() > &(AtipicialConstants::MAX_TRANSACTION_SIZE as i32) {
			return Err(TransactionError::TransactionConfiguration(
	...
	///     
	///     // Load contract AEF and manifest
	///     let aef_bytes = std::fs::read("path/to/contract.aef")?;
	...
	///         "deploy",
	///         &[
	///             ContractParameter::byte_array(aef_bytes),
...
impl<'a, P: JsonRpcProvider + 'static> AtipicialSerializable for Transaction<'a, P> {
		...
		Self: Sized,
	{
		...
		let signers: Vec<Signer> = reader
			.read_serializable_list_bounded::<Signer>(AtipicialConstants::MAX_SIGNER_SUBITEMS as usize)?;

		// Read attributes
		let attributes: Vec<TransactionAttribute> = reader
			.read_serializable_list_bounded::<TransactionAttribute>(
				AtipicialConstants::MAX_TRANSACTION_ATTRIBUTES as usize,
			)?;

		let script = reader.read_var_bytes_bounded(AtipicialConstants::MAX_TRANSACTION_SIZE as usize)?;
		...
		if tx_len > AtipicialConstants::MAX_TRANSACTION_SIZE as usize {
...
mod tests {
	...
	use crate::{
		...
		config::AtipicialConstants,
		atipicial_builder::{OracleResponse, OracleResponseCode},
		atipicial_clients::{MockClient, MockProvider},
		atipicial_protocol::{Account, RawTransaction},
	...
	fn test_try_to_array_rejects_oversized_witness_invocation_script() {
		let tx = transaction_with_witness(Witness::from_scripts(
			vec![0_u8; AtipicialConstants::MAX_TRANSACTION_SIZE as usize + 1],
			vec![],
		));
