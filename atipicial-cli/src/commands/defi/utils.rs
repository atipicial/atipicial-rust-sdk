#![allow(dead_code)]
// DeFi utilities for Atipicial CLI
//
...
use atipicial::{
	...
	atipicial_wallets::{Aep6Wallet, Wallet},
	prelude::*,
	sdk::DecimalAmount,
};
...
///
/// This represents the different Atipicial networks that can be used with the CLI
...
impl NetworkTypeCli {
	...
	pub fn to_network_string(&self) -> String {
		match self {
			...
			NetworkTypeCli::AtipicialX => "AtipicialX".to_string(),
		}
	}
}
...
pub fn get_token_address_for_network(
	...
	match network_type {
		NetworkTypeCli::MainNet => {
			// Token addresses for Atipicial Mainnet
			match token_symbol.as_str() {
				"ATC" => ScriptHash::from_str("ef4073a0f2b305a38ec4050e4d3d28bc40ea63f5").ok(),
		...
		NetworkTypeCli::TestNet => {
			// Token addresses for Atipicial Testnet
			match token_symbol.as_str() {
				"ATC" => ScriptHash::from_str("0xef4073a0f2b305a38ec4050e4d3d28bc40ea63f5").ok(),
				"GAS" => ScriptHash::from_str("0xd2a4cff31913016155e38e474a2c06d08be276cf").ok(),
				_ => None,
			}
		},
		NetworkTypeCli::AtipicialX => {
			// Professional AtipicialX token addresses for cross-chain integration
...
pub async fn get_token_decimals(
	...
	match network_type {
		...
		NetworkTypeCli::AtipicialX => {
			// Will be updated when AtipicialX info is available
		},
	}
...
mod tests {
	...
	async fn resolve_token_to_scripthash_accepts_known_symbol() {
		...
		let hash =
			resolve_token_to_scripthash_with_network("ATC", &client, NetworkTypeCli::MainNet)
				.await
				.unwrap();
