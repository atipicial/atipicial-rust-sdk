...
# AtipicialRust Monitoring Integration Guide
...
```bash
# Metrics
export ATC_METRICS_ENABLED=true
export ATC_METRICS_PORT=9090

# Tracing
export ATC_TRACING_ENABLED=true
export ATC_TRACING_ENDPOINT=http://localhost:4317

# Logging
export ATC_LOG_LEVEL=info

# Health Checks
export ATC_HEALTH_CHECK_ENABLED=true
export ATC_HEALTH_CHECK_PORT=8080
```
...
```rust
use atipicial::monitoring::metrics;
...
```rust
use atipicial::monitoring::tracing;
...
```rust
use atipicial::monitoring::health;
...
- Grafana: http://localhost:3000 (admin/atipicialrust)
...
kind: ConfigMap
metadata:
  name: atipicialrust-monitoring
data:
  ATC_METRICS_ENABLED: "true"
  ATC_METRICS_PORT: "9090"
  ATC_TRACING_ENABLED: "true"
  ATC_TRACING_ENDPOINT: "http://otel-collector:4317"
  ATC_LOG_LEVEL: "info"
```
...
```bash
export RUST_LOG=atipicial::monitoring=debug
```
