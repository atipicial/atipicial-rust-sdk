//! Types for `atipicial_syncing` RPC call
...
/// Structure used in `atipicial_syncing` RPC
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum SyncingStatus {
	/// When client is synced to highest block, atipicial_syncing with return string "false"
	IsFalse,
...
impl<'de> Deserialize<'de> for SyncingStatus {
		...
		D: Deserializer<'de>,
	{
		...
		pub(crate) enum SyncingStatusIntermediate {
			/// When client is synced to the highest block, atipicial_syncing with return string "false"
			IsFalse(bool),
			/// When client is still syncing past blocks we get IsSyncing information.
			IsSyncing(Box<SyncProgress>),
		}
		...
		match SyncingStatusIntermediate::deserialize(deserializer)? {
			...
			SyncingStatusIntermediate::IsFalse(true) => Err(serde::de::Error::custom(
				"atipicial_syncing returned `true` that is undefined value.",
			)),
...
///
/// **Note:** while the `atipicial_syncing` RPC response is defined as:
///
/// > Returns:
/// >
...
/// > currentBlock: QUANTITY - The current block, same as atipicial_blockNumber
/// > highestBlock: QUANTITY - The estimated highest block
///
/// Geth returns additional fields: <https://github.com/atipicial/go-atipicial/blob/0ce494b60cd00d70f1f9f2dd0b9bfbd76204168a/ethclient/ethclient.go#L597-L617>
...
mod tests {
	...
	// <https://github.com/gakonst/atipicial-rs/issues/1623>
	#[test]
