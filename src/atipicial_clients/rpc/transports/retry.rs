...
use crate::atipicial_clients::{JsonRpcProvider, ProviderError};
...
///
/// ```
/// use atipicial::atipicial_clients::{HttpProvider, HttpRateLimitRetryPolicy, RetryClientBuilder};
	...
	T::Error: Sync + Send + 'static + Debug,
{
	...
	///
	/// ```
	/// use atipicial::atipicial_clients::{HttpProvider, HttpRateLimitRetryPolicy, RetryClient};
	/// use std::time::Duration;
	/// use url::Url;
	///
	/// async fn demo() {
		...
		R: DeserializeOwned + Send,
	{
		...
		loop {
			...
			if should_retry {
				...
				// - `atipicial_getStorageAt`: 17
				// - `atipicial_getBlockByNumber`: 16
				// - `atipicial_newFilter`: 20
				//
