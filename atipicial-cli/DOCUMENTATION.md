...
Atipicial CLI is a comprehensive command-line tool for interacting with the Atipicial blockchain ecosystem. Built on the AtipicialRust SDK, it provides a user-friendly interface for performing a wide range of operations on the Atipicial blockchain.
...
git clone https://github.com/r3-network/atipicial-rust.git
cd atipicial-rust
...
cp target/release/atipicial-cli /usr/local/bin/
```
...
Atipicial CLI requires a configuration file to store settings such as:
...
```json
{
  "network": {
    "default": "mainnet",
    "rpc": {
      "mainnet": "http://seed1.atipicial.com:10332",
      "testnet": "http://seed1.atipicial.com:20332"
    }
  },
  "wallet": {
...
```bash
atipicial-cli wallet create [--name <NAME>] [--path <PATH>] [--password <PASSWORD>]
```
...
```bash
atipicial-cli wallet open [--name <NAME>] [--path <PATH>]
```
...
```bash
atipicial-cli wallet balance [--address <ADDRESS>] [--name <NAME>] [--path <PATH>]
```
...
```bash
atipicial-cli wallet transfer --to <ADDRESS> --asset <ASSET> --amount <AMOUNT> [--from <ADDRESS>] [--name <NAME>] [--path <PATH>]
```

Options:
- `--to`: Recipient address
- `--asset`: Asset identifier (ATC, GAS, or script hash)
...
```bash
atipicial-cli wallet export [--format <FORMAT>] [--name <NAME>] [--path <PATH>] [--output <OUTPUT_PATH>]
```

Options:
- `--format`: Export format (AEP6, WIF, etc.)
- `--name`: Name of the wallet
- `--path`: Path to the wallet file
...
```bash
atipicial-cli wallet import --input <INPUT_PATH> [--format <FORMAT>] [--name <NAME>] [--path <PATH>]
```

Options:
- `--input`: Path to the wallet file to import
- `--format`: Import format (AEP6, WIF, etc.)
...
```bash
atipicial-cli blockchain block [--hash <HASH>] [--index <INDEX>]
```
...
```bash
atipicial-cli blockchain tx --hash <HASH>
```
...
```bash
atipicial-cli blockchain asset --id <ASSET_ID>
```
...
```bash
atipicial-cli network switch --network <NETWORK>
```
...
```bash
atipicial-cli contract deploy --path <AEF_PATH> --manifest <MANIFEST_PATH> [--wallet <WALLET_PATH>] [--account <ACCOUNT>]
```

Options:
- `--path`: Path to the AEF file
...
```bash
atipicial-cli contract invoke --contract <CONTRACT_HASH> --method <METHOD_NAME> [--params <PARAMS>] [--wallet <WALLET_PATH>] [--account <ACCOUNT>]
```
...
```bash
atipicial-cli contract info --contract <CONTRACT_HASH>
```
...
```bash
atipicial-cli contract storage --contract <CONTRACT_HASH> [--key <KEY>] [--prefix <PREFIX>]
```
...
```bash
atipicial-cli defi swap --from <TOKEN> --to <TOKEN> --amount <AMOUNT> [--max-slippage <SLIPPAGE>] [--wallet <WALLET_PATH>]
```

Options:
- `--from`: Source token (ATC, GAS, or script hash)
- `--to`: Target token (ATC, GAS, or script hash)
...
```bash
atipicial-cli defi stake --token <TOKEN> --amount <AMOUNT> --platform <PLATFORM> [--duration <DURATION>] [--wallet <WALLET_PATH>]
```
...
```bash
atipicial-cli defi yield [--wallet <WALLET_PATH>] [--platform <PLATFORM>]
```
...
```bash
atipicial-cli defi pools [--platform <PLATFORM>]
```
...
```bash
atipicial-cli wallet create-multisig --min-signatures <COUNT> --pubkeys <PUBKEY1,PUBKEY2,...> [--name <NAME>]
```
...
```bash
atipicial-cli batch --input <BATCH_FILE_PATH>
```

### Scripting Support

Atipicial CLI can be used in shell scripts. All commands return appropriate exit codes and structured output that can be parsed by other tools.
...
Atipicial CLI can be used as a library in other Rust applications:

```rust
use atipicial_cli::commands::wallet;
...
For more information on API integration, see the [API documentation](https://docs.rs/atipicial-cli).
