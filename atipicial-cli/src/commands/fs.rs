...
use crate::{
	...
	errors::CliError,
	utils::{
		config::{load_config, save_config, AtipicialFsEndpoint},
		print_error, print_info, print_success,
	},
};
use clap::{Args, Subcommand};
use atipicial::atipicial_fs::{
	client::{AtipicialFsAuth, AtipicialFsConfig},
	Container, ContainerId, AtipicialFsClient, AtipicialFsService, Object, ObjectId, OwnerId,
};
use serde_json::Value;
use std::{fs, path::PathBuf};

// For compatibility with the new API
const DEFAULT_MAINNET_ENDPOINT: &str = "https://grpc.fs.atipicial.com";
const DEFAULT_TESTNET_ENDPOINT: &str = "https://grpc.testnet.fs.atipicial.com";
const DEFAULT_MAINNET_HTTP_GATEWAY: &str = "https://http.fs.atipicial.com";
const DEFAULT_TESTNET_HTTP_GATEWAY: &str = "https://http.testnet.fs.atipicial.com";
const DEFAULT_MAINNET_REST_ENDPOINT: &str = "https://rest.fs.atipicial.com";
const DEFAULT_TESTNET_REST_ENDPOINT: &str = "https://rest.testnet.fs.atipicial.com";
...
// Production-ready AtipicialFs client implementation
struct AtipicialFsClientImpl {
...
/// AtipicialFs Commands
#[derive(Args, Debug)]
pub struct FSArgs {
	/// AtipicialFs endpoint URL
	#[arg(short, long)]
...
pub enum FSCommands {
	...
	/// AtipicialFs Endpoints management and information
	Endpoints {
		#[command(subcommand)]
		command: EndpointCommands,
	},
	/// Show AtipicialFs status and connection information
	Status,
}
...
pub enum EndpointCommands {
	/// List all available AtipicialFs endpoints
	List {
		/// Network to list endpoints for (mainnet or testnet)
		#[arg(long, default_value = "mainnet")]
		network: String,
	},
	/// Test connection to a AtipicialFs endpoint
	Test {
	...
	/// Add a new AtipicialFs endpoint
	Add {
		/// Endpoint name
		#[arg(short, long)]
		name: String,
	...
	/// Remove a AtipicialFs endpoint
	Remove {
		/// Endpoint name
		#[arg(short, long)]
		name: String,
	},
	/// Set default AtipicialFs endpoint
	SetDefault {
		/// Endpoint name
		#[arg(short, long)]
		name: String,
...
/// Main handler for AtipicialFs commands
pub async fn handle_fs_command(args: FSArgs, state: &mut CliState) -> Result<(), CliError> {
	...
	let endpoint = args.endpoint.unwrap_or_else(|| {
		...
		if let Some(default_endpoint) = &config.atipicialfs.default_endpoint {
			if let Some(endpoint) =
				config.atipicialfs.endpoints.iter().find(|e| &e.name == default_endpoint)
			{
	...
	// Create a AtipicialFs client
	let client = AtipicialFsClientImpl::with_endpoint(&endpoint)?;
...
pub async fn handle_endpoint_command(command: EndpointCommands) -> Result<(), CliError> {
	match command {
		EndpointCommands::List { network } => {
			...
			let endpoints: Vec<&AtipicialFsEndpoint> =
				config.atipicialfs.endpoints.iter().filter(|e| e.network == network).collect();
			...
			print_info(&format!("AtipicialFs endpoints for {} network:", network));
			for endpoint in endpoints {
				let default = if let Some(default) = &config.atipicialfs.default_endpoint {
		...
		EndpointCommands::Test { endpoint, network, type_ } => {
			...
			let test_endpoint = match endpoint {
				...
				None => {
					...
					match network.as_str() {
						"mainnet" => "https://rest.mainnet.fs.atipicial.com".to_string(),
						"testnet" => "https://rest.testnet.fs.atipicial.com".to_string(),
						_ => {
			...
			// Test the connection
			match test_atipicialfs_connection(&test_endpoint, &type_).await {
				Ok(()) => {
		...
		EndpointCommands::Add { name, url, network, type_ } => {
			...
			if config.atipicialfs.endpoints.iter().any(|e| e.name == name) {
			...
			config.atipicialfs.endpoints.push(AtipicialFsEndpoint {
				name: name.clone(),
				url: url.clone(),
				network,
				endpoint_type: type_,
			...
			if config.atipicialfs.default_endpoint.is_none() {
				config.atipicialfs.default_endpoint = Some(name.clone());
			}

			// Save the updated config
			save_config(&config)?;

			print_success(&format!("Added AtipicialFs endpoint: {} ({})", name, url));
			Ok(())
		},
		EndpointCommands::Remove { name } => {
			...
			let endpoint_exists = config.atipicialfs.endpoints.iter().any(|e| e.name == name);
			if !endpoint_exists {
				return Err(CliError::InvalidArgument(
					"name".to_string(),
			...
			// Remove the endpoint
			config.atipicialfs.endpoints.retain(|e| e.name != name);

			// If we removed the default endpoint, update the default
			if let Some(default) = &config.atipicialfs.default_endpoint {
				if default == &name {
					config.atipicialfs.default_endpoint =
						config.atipicialfs.endpoints.first().map(|e| e.name.clone());
				}
			}
			...
			print_success(&format!("Removed AtipicialFs endpoint: {}", name));
			Ok(())
		},
		EndpointCommands::SetDefault { name } => {
			...
			print_success(&format!("Set default AtipicialFs endpoint to: {}", name));
			Ok(())
		},
	}
}
...
async fn handle_container_command(
	...
	match command {
		ContainerCommands::Create { name } => {
			...
			// Create AtipicialFs client configuration
			let config = AtipicialFsConfig::builder()
				.endpoint("https://rest.testnet.fs.atipicial.com".to_string())
			...
			// Create AtipicialFs client
			let atipicialfs_client = AtipicialFsClient::new(config).with_account(account.clone());
			...
			container.attributes.add("CreatedBy", "AtipicialRust CLI");
			container.attributes.add("CreatedAt", &chrono::Utc::now().to_rfc3339());

			// Attempt to create the container
			match atipicialfs_client.create_container(&container).await {
				Ok(created_id) => {
		...
		ContainerCommands::Get { id } => {
			...
			// Create a basic AtipicialFs client for read operations
			let config = AtipicialFsConfig::builder()
				.endpoint("https://rest.testnet.fs.atipicial.com".to_string())
				.timeout_sec(30)
				.insecure(false)
				.build();
			let atipicialfs_client = AtipicialFsClient::new(config);
			let container_id = ContainerId(id.clone());

			match atipicialfs_client.get_container(&container_id).await {
				Ok(container) => {
		...
		ContainerCommands::List => {
			...
			let config = AtipicialFsConfig::builder()
				.endpoint("https://rest.testnet.fs.atipicial.com".to_string())
				.auth(AtipicialFsAuth { wallet_address: account.get_address(), private_key: None })
				.timeout_sec(30)
				.insecure(false)
				.build();
			let atipicialfs_client = AtipicialFsClient::new(config).with_account(account.clone());

			match atipicialfs_client.list_containers().await {
				Ok(containers) => {
		...
		ContainerCommands::Delete { id } => {
			...
			match atipicialfs_client.delete_container(&container_id).await {
				Ok(success) => {
					if success {
...
async fn handle_object_command(
	...
	match command {
		ObjectCommands::Put { container, file } => {
			...
			match atipicialfs_client.put_object(&container_id, &object).await {
				Ok(object_id) => {
		...
		ObjectCommands::Get { container, id, output } => {
			...
			match atipicialfs_client.get_object(&container_id, &object_id).await {
				Ok(object) => {
		...
		ObjectCommands::List { container } => {
			...
			match atipicialfs_client.list_objects(&container_id).await {
				Ok(objects) => {
					if objects.is_empty() {
		...
		ObjectCommands::Delete { container, id } => {
			...
			match atipicialfs_client.delete_object(&container_id, &object_id).await {
				Ok(success) => {
					if success {
...
async fn handle_status_command(client: &AtipicialFsClientImpl) -> Result<(), CliError> {
	print_info("AtipicialFs Status");
	...
	match test_atipicialfs_connection(&client.grpc_endpoint, "grpc").await {
		Ok(()) => {
		...
		Err(e) => {
			...
			return Err(CliError::Network(format!("Failed to connect to AtipicialFs: {}", e)));
		},
	}
...
/// Test connection to a AtipicialFs endpoint
async fn test_atipicialfs_connection(endpoint: &str, endpoint_type: &str) -> Result<(), String> {
	match endpoint_type {
		"http" | "rest" => {
...
mod tests {
	...
	fn with_endpoint_uses_testnet_endpoints() {
		let client = AtipicialFsClientImpl::with_endpoint(DEFAULT_TESTNET_ENDPOINT).unwrap();
	...
	fn with_endpoint_propagates_http_client_creation_error() {
		let result = AtipicialFsClientImpl::with_endpoint_using("https://grpc.fs.atipicial.com", |_| {
