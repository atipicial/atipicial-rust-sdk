//! Interactive wizard for Atipicial CLI
//!
...
fn print_banner() {
	println!();
	println!("{}", "╔══════════════════════════════════════╗".cyan());
	println!("{}", format!("║{:^width$}║", "AtipicialRust Interactive Wizard", width = 38).cyan().bold());
	println!(
		"{}",
	...
	println!("{}", "╚══════════════════════════════════════╝".cyan());
	println!();
	println!("{}", "Welcome to the AtipicialRust Interactive Wizard!".green());
	println!("{}", "This tool will help you interact with the Atipicial blockchain easily.\n".white());
}
...
async fn connect_to_network() -> Result<()> {
	...
	let atipicial = match network_choice {
		0 => Atipicial::testnet().await.context("Failed to connect to TestNet")?,
		1 => Atipicial::mainnet().await.context("Failed to connect to MainNet")?,
		2 => {
			let url: String = Input::with_theme(&ColorfulTheme::default())
				.with_prompt("Enter custom RPC URL")
				.default("https://testnet1.atipicial.coz.io:443".to_string())
				.interact()?;
...
fn export_wallet() -> Result<()> {
	...
	let formats = vec!["JSON (AEP-6)", "WIF", "Mnemonic"];
...
async fn check_balance() -> Result<()> {
	...
	let address: String = Input::with_theme(&ColorfulTheme::default())
		.with_prompt("Enter Atipicial address")
	...
	// Connect to network
	let atipicial = match network {
		0 => Atipicial::testnet().await?,
		1 => Atipicial::mainnet().await?,
		_ => unreachable!(),
	};

	// Get balance
	match atipicial.get_balance(&address).await {
		Ok(balance) => {
			pb.finish_with_message("Balance fetched!");

			println!("\n{}", "Balance Information:".cyan().bold());
			println!("  📍 Address: {}", address.white());
			println!("  💎 ATC: {} tokens", balance.atipicial.to_string().green());
...
async fn generate_project() -> Result<()> {
	...
	let templates = vec![
		"Basic Atipicial dApp",
		"AEP-17 Token",
		"NFT Collection (AEP-11)",
		"DeFi Protocol",
		"Oracle Consumer",
		"Custom Template",
	];
	...
	let project_name: String = Input::with_theme(&ColorfulTheme::default())
		...
		.default("my-atipicial-project".to_string())
		.interact()?;
	...
	println!("  4. atipicial-cli deploy");
...
fn show_documentation() -> Result<()> {
	...
	let docs = vec![
		("Getting Started", "https://github.com/R3E-Network/AtipicialRust#getting-started"),
		("API Documentation", "https://docs.rs/atipicial"),
		("Examples", "https://github.com/R3E-Network/AtipicialRust/tree/master/examples"),
		("Atipicial Developer Docs", "https://developers.atipicial.com"),
		("Discord Community", "https://discord.gg/atipicial"),
	];
	...
	println!("\n💡 {}", "Tip: Use 'atipicial-cli --help' for command-line options".yellow());
