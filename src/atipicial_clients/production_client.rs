use crate::{
	...
	atipicial_error::legacy_error::{AtipicialError, AtipicialResult},
};
...
impl ProductionRpcClient {
	...
	pub async fn call(&self, method: &str, params: Vec<Value>) -> AtipicialResult<Value> {
		...
		let result: AtipicialResult<Value> = self
			.circuit_breaker
			.call(async move {
				self.pool
					.execute(move |client| {
						...
						Box::pin(async move {
							client.request(&method_inner, params_inner).await.map_err(|e| {
								AtipicialError::Network(crate::atipicial_error::NetworkError::RpcError {
									code: -1,
									message: e.to_string(),
								})
							})
	...
	pub async fn get_block_count(&self) -> AtipicialResult<u64> {
		let result = self.call("getblockcount", vec![]).await?;
		result.as_u64().ok_or_else(|| {
			AtipicialError::Serialization(crate::atipicial_error::SerializationError::InvalidFormat(
	...
	pub async fn get_block(&self, identifier: Value) -> AtipicialResult<Value> {
	...
	pub async fn get_transaction(&self, tx_hash: String) -> AtipicialResult<Value> {
		let result = self
	...
	pub async fn get_contract_state(&self, contract_hash: String) -> AtipicialResult<Value> {
		let result = self
	...
	pub async fn get_aep17_balances(&self, address: String) -> AtipicialResult<Value> {
		let result = self.call("getaep17balances", vec![Value::String(address.clone())]).await?;
	...
	pub async fn send_raw_transaction(&self, transaction_hex: String) -> AtipicialResult<Value> {
	...
	pub async fn get_health(&self) -> serde_json::Value {
		...
		serde_json::json!({
			"status": if cb_stats.current_state == crate::atipicial_clients::CircuitState::Open { "unhealthy" } else { "healthy" },
	...
	pub async fn health_check(&self) -> AtipicialResult<bool> {
	...
	fn get_cache_ttl(&self, method: &str) -> Duration {
		match method {
			...
			"getaep17balances" => Duration::from_secs(10),                 // 10 seconds - changes frequently
...
mod tests {
	...
	async fn test_production_client_creation() {
		...
		let client = ProductionRpcClient::new("https://testnet.atipicial.com:443".to_string(), config);
