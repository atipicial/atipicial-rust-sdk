...
use atipicial::{
	atipicial_clients::{public_key_to_address, public_key_to_script_hash},
	atipicial_crypto::{
		base58check_encode, try_base58check_decode, Secp256r1PublicKey, Secp256r1Signature,
	},
	atipicial_types::{AddressExtension, ScriptHash, ScriptHashExtension},
};
use ripemd::Ripemd160;
use sha2::{Digest, Sha256};

const BASE58_ALPHABET: &[u8; 58] = b"123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
const ATC_N3_ADDRESS_VERSION: u8 = 53;
...
pub enum ToolsCommands {
	...
	/// Generate Atipicial address from public key
	#[command(about = "Generate Atipicial address from public key")]
	Address {
		/// Public key (hex format)
		#[arg(short, long, help = "Public key in hex format")]
		pubkey: String,

		/// Address version (53 for Atipicial)
		#[arg(short, long, default_value = "53", help = "Address version")]
		version: u8,
	},

	/// Validate Atipicial address
	#[command(about = "Validate Atipicial address")]
	ValidateAddress {
		/// Address to validate
		#[arg(short, long, help = "Atipicial address to validate")]
		address: String,
	},
...
async fn handle_address_generation(pubkey: String, version: u8) -> Result<(), CliError> {
	print_section_header("Atipicial Address Generation");

	if version != ATC_N3_ADDRESS_VERSION {
		return Err(CliError::InvalidInput(format!(
			"Unsupported address version {}. Atipicial uses version {}.",
			version, ATC_N3_ADDRESS_VERSION
		)));
	}
...
async fn handle_validate_address(address: String) -> Result<(), CliError> {
	print_section_header("Atipicial Address Validation");

	let script_hash = address
		.address_to_script_hash()
		.map_err(|e| CliError::InvalidInput(format!("Invalid Atipicial address: {}", e)))?;
