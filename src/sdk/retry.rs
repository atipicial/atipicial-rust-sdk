//! Lightweight retry helper used by the high-level [`crate::sdk::Atipicial`] API.
//!
//! This intentionally does not duplicate the lower-level
//! [`crate::atipicial_clients::RetryClient`] / [`crate::atipicial_clients::CircuitBreaker`]
//! infrastructure — it only wraps an async closure with a bounded retry budget
//! and provider-aware [`AtipicialError`] mapping so the call sites in
//! `sdk/mod.rs` stay readable.

use std::future::Future;
use std::time::Duration;

use crate::{atipicial_clients::ProviderError, atipicial_error::unified::AtipicialError};
		...
		match operation().await {
			...
			Err(err) => {
				if !err.is_retryable() || attempt == attempts {
					return Err(AtipicialError::provider(context, err));
				}
				let retry_delay =
...
mod tests {
	...
	async fn returns_first_success_without_extra_attempts() {
		...
		let result: Result<u32, AtipicialError> =
	...
	async fn maps_final_failure_with_context() {
		let calls = Arc::new(AtomicUsize::new(0));
		let calls_clone = calls.clone();
		let result: Result<(), AtipicialError> =
			retry_network("fetch block", 2, Duration::from_millis(1), move || {
				let calls = calls_clone.clone();
				async move {
					calls.fetch_add(1, Ordering::SeqCst);
					Err::<(), _>(ProviderError::JsonRpcError(crate::atipicial_clients::JsonRpcError {
						code: 429,
			...
			retry_network("rate limited", 2, Duration::from_millis(1), move || {
				...
				async move {
					if calls.fetch_add(1, Ordering::SeqCst) == 0 {
						Err(ProviderError::JsonRpcError(crate::atipicial_clients::JsonRpcError {
							code: 429,
							message: "too many requests".to_string(),
