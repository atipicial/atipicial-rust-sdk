...
/// This example demonstrates interaction with Flamingo Finance, one of Atipicial's leading DeFi protocols.
/// It shows how to query liquidity pools, check token prices, and interact with swap functionality.
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	println!("🦩 Atipicial Flamingo Finance DeFi Protocol Example");
	println!("=================================================");

	// Connect to Atipicial MainNet (Flamingo is on MainNet)
	println!("\n📡 Connecting to Atipicial MainNet...");
	...
	println!("💡 This example demonstrates querying DeFi protocol data on Atipicial.");

	Ok(())
}

/// Query token information from a AEP-17 token contract
		...
		Ok(result) => {
			if let Some(stack_item) = result.stack.first() {
				if let Some(supply) = stack_item.as_int() {
					let decimals = 8; // Most Atipicial tokens use 8 decimals
