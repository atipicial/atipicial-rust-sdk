...
//! implementation for Atipicial blockchain, following BIP-39 (mnemonic generation)
...
//!
//! ```rust,no_run
//! use atipicial::sdk::hd_wallet::{HDWallet, HDWalletBuilder};
//! use bip39::Language;
//!
...
// use crate::atipicial_crypto::Secp256r1PublicKey;
use crate::atipicial_error::unified::{ErrorRecovery, AtipicialError};
use crate::atipicial_protocol::{Account, AccountTrait};
...
///
/// For Atipicial, the standard path is m/44'/888'/account'/0/index
...
impl DerivationPath {
	/// Create a new ATC derivation path
	///
	/// Creates a standard BIP-44 path for Atipicial blockchain.
	/// The coin type 888 is the registered number for Atipicial.
	///
	/// # Arguments
	///
	...
	pub fn new_atipicial(account: u32, index: u32) -> Result<Self, AtipicialError> {
		let account = account.checked_add(0x80000000).ok_or_else(|| AtipicialError::Wallet {
			message: format!(
		...
		Ok(Self {
			...
			coin_type: 0x80000000 + 888, // 888' for ATC
	...
	pub fn from_string(path: &str) -> Result<Self, AtipicialError> {
		...
		if parts.len() != 5 {
			return Err(AtipicialError::Wallet {
		...
		let parse_component = |s: &str| -> Result<u32, AtipicialError> {
			let hardened = s.ends_with('\'');
			let num_str = if hardened { &s[..s.len() - 1] } else { s };

			let num = num_str.parse::<u32>().map_err(|e| AtipicialError::Wallet {
			...
			num.checked_add(if hardened { 0x80000000 } else { 0 })
				.ok_or_else(|| AtipicialError::Wallet {
...
/// Atipicial wallets that support BIP-39/44.
///
/// ## Internal Structure
///
...
impl HDWallet {
	...
	pub fn generate(word_count: usize, passphrase: Option<&str>) -> Result<Self, AtipicialError> {
		...
		let mnemonic = Mnemonic::generate(word_count).map_err(|e| AtipicialError::Wallet {
	...
	pub fn from_phrase(
		...
		let mnemonic = Mnemonic::parse_in(language, phrase).map_err(|e| AtipicialError::Wallet {
	...
	///
	/// Derives a Atipicial account at the specified BIP-44 path. Accounts are
	...
	/// # Returns
	///
	/// A Atipicial `Account` that can be used for transactions
	///
	/// # Performance
	///
	...
	pub fn derive_account(&mut self, path: &str) -> Result<Account, AtipicialError> {
		...
		use crate::atipicial_crypto::{wif_from_private_key, Secp256r1PrivateKey};

		let private_key =
			Secp256r1PrivateKey::from_bytes(&derived_key.key).map_err(|e| AtipicialError::Wallet {
		...
		let account = Account::from_wif(&wif).map_err(|e| AtipicialError::Wallet {
	...
	pub fn derive_accounts(
		...
		account_index: u32,
		count: u32,
	) -> Result<Vec<Account>, AtipicialError> {
		let mut accounts = Vec::new();

		for i in 0..count {
			let index = account_index.checked_add(i).ok_or_else(|| AtipicialError::Wallet {
				message: format!(
	...
	fn derive_key(&self, path: &DerivationPath) -> Result<ExtendedPrivateKey, AtipicialError> {
	...
	pub fn export_encrypted(&self, password: &str) -> Result<String, AtipicialError> {
		...
		let plaintext = serde_json::to_vec(&wallet_data).map_err(|e| AtipicialError::Wallet {
			message: format!("Failed to serialize wallet: {e}"),
			source: Some(Box::new(e)),
			recovery: ErrorRecovery::new(),
		})?;

		// Derive key using standard Atipicial scrypt parameters and random salt
		let scrypt_def = crate::atipicial_types::ScryptParamsDef::default();
		let params =
		...
		serde_json::to_string_pretty(&encrypted).map_err(|e| AtipicialError::Wallet {
	...
	pub fn import_encrypted(json: &str, password: &str) -> Result<Self, AtipicialError> {
		...
		let wallet_data: HDWalletData = if let Ok(encrypted_payload) = encrypted_payload {
			...
			let salt = general_purpose::STANDARD
				...
				.map_err(|e| AtipicialError::Wallet {
			...
			let plaintext = match encrypted_payload.version {
				...
				HD_WALLET_LEGACY_ECB_VERSION => {
					...
					let plaintext = Aes256EcbDec::new(&key.into())
						...
						.map_err(|_| AtipicialError::Wallet {
			...
			let plaintext_str = String::from_utf8(plaintext).map_err(|e| AtipicialError::Wallet {
				message: format!("Decrypted data is not valid UTF-8: {e}"),
				source: Some(Box::new(e)),
				recovery: ErrorRecovery::new(),
			})?;

			serde_json::from_str::<HDWalletData>(&plaintext_str).map_err(|e| AtipicialError::Wallet {
			...
			serde_json::from_str::<HDWalletData>(json).map_err(|e| AtipicialError::Wallet {
...
fn encrypted_wallet_error(message: impl Into<String>) -> AtipicialError {
	AtipicialError::Wallet { message: message.into(), source: None, recovery: ErrorRecovery::new() }
}
...
fn derive_hd_wallet_key_material(
	...
	len: usize,
) -> Result<Vec<u8>, AtipicialError> {
	use scrypt::scrypt;

	let mut key_material = vec![0u8; len];
	scrypt(password.as_bytes(), salt, params, &mut key_material).map_err(|e| AtipicialError::Wallet {
...
fn hd_wallet_auth_tag(
	version: u8,
	params: &crate::atipicial_types::ScryptParamsDef,
	salt: &[u8],
	nonce: &[u8; HD_WALLET_NONCE_LEN],
	ciphertext: &[u8],
	mac_key: &[u8; 32],
) -> Result<[u8; HD_WALLET_TAG_LEN], AtipicialError> {
...
impl ExtendedPrivateKey {
	/// Create from seed
	fn from_seed(seed: &[u8]) -> Result<Self, AtipicialError> {
		let mut mac =
			Hmac::<Sha512>::new_from_slice(b"Nist256p1 seed").map_err(|e| AtipicialError::Wallet {
	...
	/// Derive child key
	fn derive_child(&self, index: u32) -> Result<Self, AtipicialError> {
		let mut mac =
			Hmac::<Sha512>::new_from_slice(&self.chain_code).map_err(|e| AtipicialError::Wallet {
		...
		if index >= 0x80000000 {
			...
			let secret_key =
				p256::SecretKey::from_slice(&self.key).map_err(|e| AtipicialError::Wallet {
...
struct EncryptedHDWalletData {
	version: u8,
	scrypt: crate::atipicial_types::ScryptParamsDef,
	/// base64-encoded salt
	salt: String,
