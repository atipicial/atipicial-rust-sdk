...
/// This example demonstrates how to use a local signer to sign messages in Atipicial.
#[tokio::main]
async fn main() -> Result<(), Box<dyn Error>> {
	println!("🔐 Atipicial Local Signer Example");
	println!("============================");
	...
	let account = match std::env::var("ATC_WIF") {
		Ok(wif) => {
			...
			println!("   ✅ Account loaded from ATC_WIF");
			account
		},
		Err(_) => {
			println!("   ⚠️  ATC_WIF not set; generating a random account for demo");
			Account::create()?
		},
	};
	...
	// 3. Connect to Atipicial testnet
	println!("\n3. Connecting to Atipicial testnet:");
	let provider = HttpProvider::new("https://testnet1.atipicial.coz.io:443")?;
	let client = RpcClient::new(provider);

	// Test connection
	let version = client.get_version().await?;
	println!("   ✅ Connected to Atipicial node");
	...
	let atipicial_token = H160::from_hex("ef4073a0f2b305a38ec4050e4d3d28bc40ea63f5")?;
	let gas_token = H160::from_hex("d2a4cff31913016155e38e474a2c06d08be276cf")?;

	// Get ATC balance
	let atipicial_balance = client
		.invoke_function(
			&atipicial_token,
			"balanceOf".to_string(),
	...
	println!("   💰 ATC Balance: {:?}", atipicial_balance.stack.first());
	println!("   ⛽ GAS Balance: {:?}", gas_balance.stack.first());

	// 5. Create and sign a transaction
	println!("\n5. Creating and signing a transaction:");

	// Create a simple script that calls ATC's symbol method
	let script = ScriptBuilder::new().contract_call(&atipicial_token, "symbol", &[], None)?.to_bytes();
	...
	println!("\n6. Message signing:");
	let message = "Hello, Atipicial blockchain!";
	...
	if let Some(key_pair) = account.key_pair() {
		...
		println!("   ℹ️  Signature verification is performed by the Atipicial network");
	}
	...
	println!("   📊 Total ATC supply: {:?}", invoke_result.stack.first());
