...
use crate::{
	...
	atipicial_clients::{APITrait, Http, JsonRpcProvider, ProviderError, RwClient},
};
...
impl FromStr for AtipicialClient {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		match first_segment.to_lowercase().as_str() {
			"atipicial" => Ok(AtipicialClient::ATC),
			_ => Err(ProviderError::UnsupportedNodeClient),
		}
	}
}

/// An abstract provider for interacting with the [Atipicial JSON RPC
/// API](https://github.com/atipicial/wiki/JSON-RPC). Must be instantiated
...
/// use atipicial::atipicial_config::AtipicialConstants;
///
/// async fn foo() -> Result<(), Box<dyn std::error::Error>> {
///     let provider = HttpProvider::new(AtipicialConstants::SEED_1)?;
...
impl<P: JsonRpcProvider> RpcClient<P> {
	...
	pub async fn node_client(&self) -> Result<AtipicialVersion, ProviderError> {
...
impl<P: JsonRpcProvider> APITrait for RpcClient<P> {
	...
	async fn network(&self) -> Result<u32, ProviderError> {
		if ATCCONFIG.lock().map_err(|_| ProviderError::LockError)?.network.is_none() {
	...
	/// - Returns: The request object
	async fn get_block(&self, block_hash: H256, full_tx: bool) -> Result<AtipicialBlock, ProviderError> {
		Ok(if full_tx {
	...
	async fn get_block_by_hash(
		...
		hash: &str,
		full_tx: bool,
	) -> Result<AtipicialBlock, ProviderError> {
	...
	/// - Parameter blockHash: The block hash
	/// - Returns: The request object
	async fn get_block_header(&self, block_hash: H256) -> Result<AtipicialBlock, ProviderError> {
	...
	/// - Parameter blockIndex: The block index
	/// - Returns: The request object
	async fn get_block_header_by_index(&self, index: u32) -> Result<AtipicialBlock, ProviderError> {
	...
	/// Gets the public key list of current Atipicial committee members.
	/// - Returns: The request object
	...
	/// - Returns: The request object
	async fn get_version(&self) -> Result<AtipicialVersion, ProviderError> {
		self.request("getversion", Vec::<AtipicialVersion>::new()).await
	}

	/// Broadcasts a transaction over the ATC network.
	...
	/// Broadcasts a new block over the ATC network.
	...
	async fn broadcast_block(&self, block: AtipicialBlock) -> Result<bool, ProviderError> {
	...
	async fn create_contract_deployment_transaction(
		...
		let aef_bytes = aef
			.try_to_array()
			.map_err(|e| ProviderError::ParseError(format!("Failed to serialize AEF: {}", e)))?;
	...
	/// Verifies whether the address is a valid ATC address.
	...
	/// - Returns: The request object
	async fn import_priv_key(&self, priv_key: String) -> Result<AtipicialAddress, ProviderError> {
	...
	async fn calculate_network_fee(
		&self,
		tx_base64: String,
	) -> Result<AtipicialNetworkFee, ProviderError> {
		self.request(
			"calculatenetworkfee",
	...
	/// - Returns: The request object
	async fn list_address(&self) -> Result<Vec<AtipicialAddress>, ProviderError> {
		self.request("listaddress", Vec::<AtipicialAddress>::new()).await
	}
	...
	/// - Parameters:
	///   - tokenHash: The token hash of the AEP-17 contract
	...
	/// Gets the balance of all AEP-17 token assets in the specified script hash.
	/// - Parameter scriptHash: The account's script hash
	/// - Returns: The request object
	async fn get_aep17_balances(&self, script_hash: H160) -> Result<Aep17Balances, ProviderError> {
		self.request("getaep17balances", [script_hash.to_address().to_value()].to_vec())
			.await
	}

	/// Gets all the AEP-17 transaction information occurred in the specified script hash.
	/// - Parameter scriptHash: The account's script hash
	/// - Returns: The request object
	async fn get_aep17_transfers(
		&self,
		script_hash: H160,
	) -> Result<Aep17Transfers, ProviderError> {
		let params = json!([script_hash.to_address()]);
		self.request("getaep17transfers", params).await
	}

	/// Gets all the AEP17 transaction information occurred in the specified script hash since the specified time.
	/// - Parameters:
	///   - scriptHash: The account's script hash
	///   - from: The timestamp transactions occurred since
	/// - Returns: The request object
	async fn get_aep17_transfers_from(
		&self,
		script_hash: H160,
		from: u64,
	) -> Result<Aep17Transfers, ProviderError> {
		self.request("getaep17transfers", json!([script_hash.to_address(), from])).await
	}

	/// Gets all the AEP17 transaction information occurred in the specified script hash in the specified time range.
	/// - Parameters:
	///   - scriptHash: The account's script hash
	///   - from: The start timestamp
	///   - to: The end timestamp
	/// - Returns: The request object
	async fn get_aep17_transfers_range(
		&self,
		script_hash: H160,
		from: u64,
		to: u64,
	...
	/// Gets all AEP-11 balances of the specified account.
	/// - Parameter scriptHash: The account's script hash
	/// - Returns: The request object
	async fn get_aep11_balances(&self, script_hash: H160) -> Result<Aep11Balances, ProviderError> {
		let params = json!([script_hash.to_address()]);
		self.request("getaep11balances", params).await
	}

	/// Gets all AEP-11 transaction of the given account.
	/// - Parameter scriptHash: The account's script hash
	/// - Returns: The request object
	async fn get_aep11_transfers(
		&self,
		script_hash: H160,
	) -> Result<Aep11Transfers, ProviderError> {
		let params = json!([script_hash.to_address()]);
		self.request("getaep11transfers", params).await
	}

	/// Gets all AEP-11 transaction of the given account since the given time.
	/// - Parameters:
	///   - scriptHash: The account's script hash
	///   - from: The date from when to report transactions
	/// - Returns: The request object
	async fn get_aep11_transfers_from(
		&self,
		script_hash: H160,
		from: u64,
	) -> Result<Aep11Transfers, ProviderError> {
	...
	/// Gets all AEP-11 transactions of the given account in the time span between `from` and `to`.
	/// - Parameters:
	///   - scriptHash: The account's script hash
	///   - from: The start timestamp
	///   - to: The end timestamp
	/// - Returns: The request object
	async fn get_aep11_transfers_range(
		&self,
		script_hash: H160,
		from: u64,
		to: u64,
	...
	/// Gets the properties of the token with `tokenId` from the AEP-11 contract with `scriptHash`.
	///
	/// The properties are a mapping from the property name string to the value string.
	/// The value is plain text if the key is one of the properties defined in the AEP-11 standard.
	/// Otherwise, the value is a Base64-encoded byte array.
	///
	/// To receive custom property values that consist of nested types (e.g., Maps or Arrays) use ``invokeFunction(_:_:_:)``  to directly invoke the method `properties` of the AEP-11 smart contract.
	/// - Parameters:
	///   - scriptHash: The account's script hash
	///   - tokenId: The ID of the token as a hexadecimal string
	/// - Returns: The request object
	async fn get_aep11_properties(
		&self,
		script_hash: H160,
		token_id: &str,
	) -> Result<HashMap<String, String>, ProviderError> {
		let params = json!([script_hash.to_address(), token_id]);
		self.request("getaep11properties", params).await
	}
	...
	///   - countFindResultItems: The number of results. An upper limit is defined in the Atipicial core
	/// - Returns: The request object
	...
	///
	/// The results are limited to `count` items. If `count` is greater than `MaxIteratorResultItems` in the Atipicial Node's configuration file, this request fails.
	/// - Parameters:
	...
	fn import_private_key<'life0, 'async_trait>(
		&'life0 self,
		wif: String,
	) -> Pin<Box<dyn Future<Output = Result<AtipicialAddress, Self::Error>> + Send + 'async_trait>>
	where
		'life0: 'async_trait,
		Self: 'async_trait,
	{
	...
	fn get_block_header_hash<'life0, 'async_trait>(
		&'life0 self,
		hash: H256,
	) -> Pin<Box<dyn Future<Output = Result<AtipicialBlock, Self::Error>> + Send + 'async_trait>>
	where
		'life0: 'async_trait,
		Self: 'async_trait,
	{
...
impl RpcClient<crate::atipicial_clients::Ipc> {
	...
	pub async fn connect_ipc(path: impl AsRef<std::path::Path>) -> Result<Self, ProviderError> {
		let ipc = crate::atipicial_clients::Ipc::connect(path).await?;
		Ok(Self::new(ipc))
	}
}
...
mod tests {
	...
	use crate::{
		...
		atipicial_clients::{APITrait, MockProvider},
	...
	fn encodable_test_aef() -> AefFile {
		AefFile::new(Some("test-compiler".to_string()), "", vec![0x11, 0x40], vec![0; 4])
	}

	#[tokio::test]
	async fn create_contract_deployment_transaction_rejects_invalid_aef_before_request() {
		let provider = MockProvider::new();
		let client = RpcClient::new(provider.clone());
		let mut aef = encodable_test_aef();
		aef.compiler = Some("x".repeat(65));

		let err = client
			.create_contract_deployment_transaction(aef, ContractManifest::default(), vec![])
			.await
			.unwrap_err();
		assert_parse_error(err, "AEF");
		assert!(provider.take_requests().is_empty());
	}

	#[tokio::test]
	async fn create_contract_update_transaction_rejects_invalid_aef_before_request() {
		let provider = MockProvider::new();
		let client = RpcClient::new(provider.clone());
		let mut aef = encodable_test_aef();
		aef.compiler = Some("x".repeat(65));
