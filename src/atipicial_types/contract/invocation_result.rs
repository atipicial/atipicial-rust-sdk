...
use crate::TypeError;
use atipicial::prelude::{deserialize_script_hash, serialize_script_hash, ContractParameter, StackItem};
...
impl<'de> Deserialize<'de> for AtipicialVMStateType {
		...
		D: Deserializer<'de>,
	{
		...
		if value.is_empty() {
			return Ok(AtipicialVMStateType::None); // Handle empty string as `None`
		}
		let value_lower = value.to_lowercase();

		match value_lower.as_str() {
			"none" => Ok(AtipicialVMStateType::None),
			"halt" => Ok(AtipicialVMStateType::Halt),
			"fault" => Ok(AtipicialVMStateType::Fault),
			"break" => Ok(AtipicialVMStateType::Break),
			_ => Err(de::Error::invalid_value(
				Unexpected::Str(&value),
				&"a valid AtipicialVMStateType string",
			)),
		}
	}
}
...
impl InvocationResult {
	...
	pub fn has_state_fault(&self) -> bool {
		!matches!(self.state, AtipicialVMStateType::Halt)
	}

	pub fn get_first_stack_item(&self) -> Result<&StackItem, TypeError> {
		if self.stack.is_empty() {
			return Err(TypeError::IndexOutOfBounds(
                "The stack is empty. This means that no items were left on the AtipicialVM stack after this invocation."
                    .to_string(),
            ));
		}
	...
	pub fn get_stack_item(&self, index: usize) -> Result<&StackItem, TypeError> {
		if index >= self.stack.len() {
			return Err(TypeError::IndexOutOfBounds(format!(
				"There were only {} items left on the AtipicialVM stack after this invocation",
				self.stack.len()
			)));
		}
		Ok(&self.stack[index])
...
impl Default for InvocationResult {
	fn default() -> Self {
		Self {
			...
			state: AtipicialVMStateType::Halt,
...
mod tests {
	...
	fn deserialize_missing_state_defaults_to_none() {
		...
		assert_eq!(result.state, AtipicialVMStateType::None);
	}
	...
	fn non_halt_states_are_treated_as_faults() {
		let mut result = InvocationResult { state: AtipicialVMStateType::None, ..Default::default() };
		assert!(result.has_state_fault());

		result.state = AtipicialVMStateType::Break;
		assert!(result.has_state_fault());

		result.state = AtipicialVMStateType::Fault;
		assert!(result.has_state_fault());

		result.state = AtipicialVMStateType::Halt;
		assert!(!result.has_state_fault());
	}
}
