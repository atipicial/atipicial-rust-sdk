use crate::atipicial_error::legacy_error::{AtipicialError, AtipicialResult};
use std::{
	sync::{
...
impl CircuitBreaker {
	...
	pub async fn call<F, T>(&self, operation: F) -> AtipicialResult<T>
	where
		F: std::future::Future<Output = AtipicialResult<T>>,
	{
		...
		if !self.should_allow_request().await {
			...
			return Err(AtipicialError::Network(crate::atipicial_error::NetworkError::RateLimitExceeded));
		}
...
mod tests {
	...
	async fn test_circuit_breaker_closed_state() {
		...
		for _ in 0..5 {
			let result = cb.call(async { Ok::<(), AtipicialError>(()) }).await;
			assert!(result.is_ok());
		}
	...
	async fn test_circuit_breaker_opens_on_failures() {
		...
		for _ in 0..3 {
			let result = cb
				.call(async {
					Err::<(), AtipicialError>(AtipicialError::Network(
						crate::atipicial_error::NetworkError::ConnectionFailed("test".to_string()),
					))
				})
				.await;
	...
	async fn test_circuit_breaker_stats() {
		...
		// Make some requests
		let _ = cb.call(async { Ok::<(), AtipicialError>(()) }).await;
		let _ = cb
			.call(async {
				Err::<(), AtipicialError>(AtipicialError::Network(
					crate::atipicial_error::NetworkError::ConnectionFailed("test".to_string()),
