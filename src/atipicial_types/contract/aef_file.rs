...
use crate::{
	codec::{CodecError, Decoder, Encoder, AtipicialSerializable, VarSizeTrait},
	crypto::HashableForVec,
	atipicial_types::StringExt,
	TypeError,
};
use atipicial::prelude::{Bytes, ContractParameter, StackItem};
/*
┌───────────────────────────────────────────────────────────────────────┐
...
impl From<AefFile> for ContractParameter {
	fn from(val: AefFile) -> ContractParameter {
		ContractParameter::try_from_aef_file(&val).unwrap_or_else(|err| {
			tracing::warn!(
				error = %err,
				"Failed to convert owned AEF file to contract parameter via safe path; falling back to legacy serializer"
			);
...
impl AefFile {
	...
	fn compute_checksum(file: &AefFile) -> Result<Bytes, TypeError> {
	...
	fn encode_without_checksum(&self, writer: &mut Encoder) -> Result<(), TypeError> {
		if self.source_url.len() > Self::MAX_SOURCE_URL_SIZE {
			return Err(TypeError::InvalidEncoding(format!(
				"AEF source URL exceeds maximum length of {} bytes",
		...
		if self.script.len() > Self::MAX_SCRIPT_LENGTH {
			return Err(TypeError::InvalidEncoding(format!(
				"AEF script exceeds maximum length of {} bytes",
		...
		writer.write_fixed_string(&self.compiler, Self::COMPILER_SIZE).map_err(|e| {
			TypeError::InvalidEncoding(format!("Failed to serialize AEF compiler: {}", e))
		})?;
		writer.write_var_string(&self.source_url);
		writer.write_u8(0);
		writer.write_var_int(self.method_tokens.len() as i64).map_err(|e| {
			TypeError::InvalidEncoding(format!("Failed to serialize AEF method token count: {}", e))
		})?;
		for method_token in &self.method_tokens {
			method_token.try_encode(writer).map_err(|e| {
				TypeError::InvalidEncoding(format!("Failed to serialize AEF method token: {}", e))
			})?;
		}
		writer.write_u16(0);
		writer.write_var_bytes(&self.script).map_err(|e| {
			TypeError::InvalidEncoding(format!("Failed to serialize AEF script: {}", e))
		})?;
		Ok(())
	}
	...
	fn encode_legacy(&self, writer: &mut Encoder) {
		...
		if let Err(err) = writer.write_fixed_string(&self.compiler, Self::COMPILER_SIZE) {
			tracing::warn!(error = %err, "Failed to serialize AEF compiler");
		}
		writer.write_var_string(&self.source_url);
		writer.write_u8(0);
		if let Err(err) = writer.write_serializable_variable_list(&self.method_tokens) {
			tracing::warn!(error = %err, "Failed to serialize AEF method tokens");
		}
		writer.write_u16(0);
		if let Err(err) = writer.write_var_bytes(&self.script) {
			tracing::warn!(error = %err, "Failed to serialize AEF script");
		}
		...
		match self.compute_checksum_for_payload() {
			Ok(computed_checksum) => {
				if let Err(err) = self.encode_without_checksum(writer) {
					tracing::warn!(
						error = %err,
						"Failed to serialize AEF payload after checksum recompute; falling back to legacy encoder"
					);
			...
			Err(err) => {
				tracing::warn!(
					error = %err,
					"Failed to serialize AEF via safe payload path; falling back to legacy encoder"
				);
				self.encode_legacy(writer);
			},
		}
	...
	fn read_from_file(file: &str) -> Result<Self, TypeError> {
		let file_bytes = std::fs::read(file)
			.map_err(|e| TypeError::InvalidArgError(format!("Failed to read AEF file: {}", e)))?;

		if file_bytes.len() > 0x100000 {
			return Err(TypeError::InvalidArgError("AEF file is too large".to_string()));
		}

		let mut reader = Decoder::new(&file_bytes);
		reader.read_serializable().map_err(|e| {
			TypeError::InvalidEncoding(format!("Failed to deserialize AEF file: {}", e))
		})
	}

	/// Deserializes a AEF file from a byte array
	///
	/// # Arguments
	///
	...
	/// # Returns
	///
	/// A `Result` containing the deserialized AEF file or a `TypeError`
		...
		if let StackItem::ByteString { value: bytes } = item {
			...
			reader.read_serializable().map_err(|e| {
				TypeError::InvalidEncoding(format!(
					"Failed to deserialize AEF from stack item: {}",
					e
				))
			})
		} else {
	...
	pub fn try_encode(&self, writer: &mut Encoder) -> Result<(), TypeError> {
		...
		if self.checksum.len() != Self::CHECKSUM_SIZE {
			return Err(TypeError::InvalidEncoding("AEF checksum length is invalid".to_string()));
		}
...
impl AtipicialSerializable for AefFile {
	...
	fn to_array(&self) -> Vec<u8> {
		self.try_to_array().unwrap_or_else(|err| {
			tracing::warn!(
				error = %err,
				"Failed to serialize AEF via safe path; falling back to legacy encoder"
			);
			let mut writer = Encoder::new();
...
impl AtipicialSerializable for MethodToken {
	type Error = TypeError;
...
mod tests {
	...
	fn create_valid_serialized_aef_bytes() -> Vec<u8> {
		let script = vec![0x11, 0x40];
		let mut writer = Encoder::new();
		writer.write_u32(AefFile::MAGIC);
		writer
			.write_fixed_string(&Some("test-compiler".to_string()), AefFile::COMPILER_SIZE)
			.unwrap();
		...
		bytes.extend_from_slice(&checksum[..AefFile::CHECKSUM_SIZE]);
		bytes
	}

	fn create_encodable_aef() -> AefFile {
		AefFile::new(Some("test-compiler".to_string()), String::new(), vec![0x11, 0x40], vec![0; 4])
	}

	#[test]
	fn test_deserialize_preserves_checksum_and_roundtrips() {
		let bytes = create_valid_serialized_aef_bytes();
		let aef = AefFile::deserialize(&bytes).unwrap();

		assert_eq!(aef.checksum.len(), AefFile::CHECKSUM_SIZE);
		assert_eq!(aef.try_to_array().unwrap(), bytes);
	}

	#[test]
	fn test_to_array_repairs_checksum_for_encodable_aef() {
		let legacy_aef = AefFile {
			compiler: Some("test-compiler".to_string()),
			source_url: String::new(),
			method_tokens: vec![],
			script: vec![0x11, 0x40],
			checksum: vec![0; AefFile::CHECKSUM_SIZE],
		};

		let canonical_aef = AefFile::new(
		...
		assert_eq!(legacy_aef.to_array(), canonical_aef.to_array());
		assert!(AefFile::deserialize(&legacy_aef.to_array()).is_ok());
	}
	...
	fn test_try_to_array_rejects_invalid_checksum_length() {
		let aef = AefFile {
			...
			method_tokens: vec![],
			script: vec![1, 2, 3],
			checksum: vec![0; AefFile::CHECKSUM_SIZE - 1],
		};
	...
	#[test]
	fn test_aef_size_matches_serialized_length() {
		let aef = create_encodable_aef();
		assert_eq!(aef.size(), aef.try_to_array().unwrap().len());
	}
	...
	fn test_try_to_array_rejects_source_url_longer_than_max() {
		let aef = AefFile {
			compiler: Some("test-compiler".to_string()),
			source_url: "x".repeat(AefFile::MAX_SOURCE_URL_SIZE + 1),
			method_tokens: vec![],
			script: vec![1, 2, 3],
			checksum: vec![0; 4],
		};
	...
	fn test_try_to_array_rejects_script_longer_than_max() {
		let aef = AefFile {
			...
			source_url: String::new(),
			method_tokens: vec![],
			script: vec![0; AefFile::MAX_SCRIPT_LENGTH + 1],
			checksum: vec![0; 4],
		};
	...
	fn test_try_to_array_rejects_compiler_longer_than_fixed_width() {
		let aef = AefFile {
			compiler: Some("x".repeat(AefFile::COMPILER_SIZE + 1)),
			source_url: String::new(),
			method_tokens: vec![],
			script: vec![1, 2, 3],
			checksum: vec![0; 4],
	...
	#[test]
	fn test_try_to_array_matches_legacy_for_encodable_aef() {
		let aef = create_encodable_aef();
		assert_eq!(aef.try_to_array().unwrap(), aef.to_array());
	}
}
