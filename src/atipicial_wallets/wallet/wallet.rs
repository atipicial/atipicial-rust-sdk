...
use crate::{
	atipicial_builder::{Transaction, TransactionBuilder, Witness},
	atipicial_clients::{APITrait, JsonRpcProvider, ProviderError, RpcClient},
	atipicial_config::AtipicialConstants,
	atipicial_crypto::{CryptoError, HashableForVec, KeyPair, Secp256r1Signature},
	atipicial_protocol::{Account, AccountTrait, UnclaimedGas},
	atipicial_types::{
	...
	atipicial_wallets::{AEP6Account, Aep6Wallet, WalletError, WalletTrait},
};
use scrypt::Params;
...
impl Wallet {
		...
		Params::new(self.scrypt_params.log_n, self.scrypt_params.r, self.scrypt_params.p, 32)
			.unwrap_or_else(|e| {
				tracing::warn!(
					...
					"Invalid scrypt params; falling back to Atipicial defaults"
				);
				Params::new(
					AtipicialConstants::SCRYPT_LOG_N,
					AtipicialConstants::SCRYPT_R,
					AtipicialConstants::SCRYPT_P,
					32,
				)
				.unwrap_or_else(|e| {
					tracing::error!(
						error = %e,
						"Atipicial default scrypt parameters are invalid; falling back to scrypt recommended params"
					);
					Params::recommended()
				})
			})
	...
	/// Converts the wallet to a AEP6Wallet format.
	pub fn to_aep6(&self) -> Result<Aep6Wallet, WalletError> {
		let accounts = self
			.accounts
			.values()
			.map(AEP6Account::from_account)
			.collect::<Result<Vec<AEP6Account>, _>>()?;
	...
	/// Creates a wallet from a AEP6Wallet format.
	pub fn from_aep6(aep6: Aep6Wallet) -> Result<Self, WalletError> {
		let accounts = aep6
			.accounts()
			.iter()
			.map(AEP6Account::to_account)
			.collect::<Result<Vec<_>, _>>()?;

		if accounts.is_empty() {
			tracing::warn!("No accounts found in AEP6 wallet");
		...
		let mut wallet = Self {
			name: aep6.name().clone(),
			version: aep6.version().clone(),
			scrypt_params: aep6.scrypt().clone(),
			accounts: accounts
				.into_iter()
				.map(|account| (account.get_script_hash(), account))
				.collect(),
			default_account,
			extra: aep6.extra.clone(),
		};
	...
	pub fn save_to_file(&self, path: PathBuf) -> Result<(), WalletError> {
		...
		let aep6 = self.to_aep6()?;

		// Encode as JSON
		let json = serde_json::to_string(&aep6).map_err(|e| {
			WalletError::AccountState(format!("Failed to serialize wallet to JSON: {e}"))
		})?;

		// Write to file at path. AEP-6 wallets hold encrypted key material, so
...
impl Wallet {
	...
	pub fn open_wallet(path: &Path, password: &str) -> Result<Self, WalletError> {
		...
		let aep6_wallet: Aep6Wallet = serde_json::from_str(&wallet_json).map_err(|e| {
			WalletError::DeserializationError(format!("Failed to parse wallet JSON: {e}"))
		})?;

		// Convert to Wallet
		let wallet = Wallet::from_aep6(aep6_wallet)?;
	...
	fn network(&self) -> u32 {
		...
		self.extra
			...
			.and_then(|extra| {
				extra
					.get("network")
					.map(|n| n.parse::<u32>().unwrap_or(AtipicialConstants::MAGIC_NUMBER_MAINNET))
			})
			.unwrap_or(AtipicialConstants::MAGIC_NUMBER_MAINNET)
	}
...
mod tests {
	use crate::{
		...
		atipicial_wallets::{AEP6Account, Aep6Wallet, Wallet, WalletError, WalletTrait},
		ScryptParamsDef,
	};
	use primitive_types::H160;
	...
	fn test_create_default_wallet() {
		...
		assert_eq!(&wallet.name, "AtipicialWallet");
	...
	#[test]
	fn test_to_aep6_rejects_unencrypted_accounts_instead_of_dropping_them() {
		let wallet = Wallet::try_new().expect("Should create wallet with default account");

		let result = wallet.to_aep6();
		assert!(matches!(
			result,
	...
	#[test]
	fn test_from_aep6_rejects_empty_wallet() {
		let aep6_wallet = Aep6Wallet::new(
			"Empty".to_string(),
		...
		let err = Wallet::from_aep6(aep6_wallet).unwrap_err();
		assert!(matches!(err, WalletError::NoAccounts));
	}

	#[test]
	fn test_from_aep6_surfaces_invalid_account_errors() {
		let aep6_wallet = Aep6Wallet::new(
			"Invalid".to_string(),
			Wallet::CURRENT_VERSION.to_string(),
			ScryptParamsDef::default(),
			vec![AEP6Account::new(String::new(), None, true, false, None, None, None)],
			None,
		);
