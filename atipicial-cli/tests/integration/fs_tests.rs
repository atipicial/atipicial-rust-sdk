...
/// Test the AtipicialFs endpoints list command
#[test]
fn test_fs_endpoints_list() {
	...
	assert_output_contains(&output, "AtipicialFs endpoints for mainnet network:");

	// Test testnet endpoints list
	let output = cli.run(&["fs", "endpoints", "list", "--network", "testnet"]);

	assert_success(&output);
	assert_output_contains(&output, "AtipicialFs endpoints for testnet network:");
}

/// Test the AtipicialFs status command without a wallet
#[test]
...
/// Test the AtipicialFs endpoint test command
#[test]
fn test_fs_endpoints_test() {
	...
	let output = cli.run(&[
		...
		"test",
		"--endpoint",
		"https://http.mainnet.fs.atipicial.com",
		"--type",
		"http",
	]);
...
/// Test the AtipicialFs container commands
#[test]
...
/// Test the AtipicialFs object commands
#[test]
fn test_fs_object_operations() {
	...
	std::io::Write::write_all(&mut temp_file, b"This is test content for AtipicialFs").unwrap();
...
/// Test the AtipicialFs ACL commands
#[test]
