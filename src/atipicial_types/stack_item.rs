...
/// This module defines the `StackItem` enum and `MapEntry` struct, which are used to represent items on the Atipicial virtual machine stack.
...
use atipicial::prelude::{Address, ScriptHashExtension};

/// The `StackItem` enum represents an item on the Atipicial virtual machine stack.
///
/// # Memory Layout Considerations
///
/// This enum is designed for use in Atipicial VM stack operations and JSON-RPC serialization.
...
/// Atipicial's JSON-RPC response format. Any changes to variant structure must maintain
/// backward compatibility with the Atipicial node RPC API.
...
pub enum StackItem {
	...
	///
	/// Arrays are commonly used in Atipicial VM execution results and smart contract return values.
	...
	///
	/// # Note
	/// Similar to `Array`, but represents a struct type in the Atipicial VM. The memory layout
	...
	/// iteration order (required for Atipicial VM consistency) and reduce memory overhead.
...
impl StackItem {
	...
	/// # Example
	/// ```rust
	/// use atipicial::atipicial_types::{StackItem, MapEntry};
	///
