//! Mock provider example for Atipicial blockchain
//!
...
// Mock provider for testing Atipicial RPC calls
use serde_json::json;
...
async fn main() -> eyre::Result<()> {
	println!("🧪 Atipicial Mock Provider Example");
	println!("===============================");
	...
	mock.add_response(
		...
		json!({
			"tcpport": 10333,
			"nonce": 388190803,
			"useragent": "/Atipicial:3.7.4+44c8cd9669beffd8460a56aedf81a53b47ff5b5f/",
			"protocol": {
				"addressversion": 53,
				"network": 860833102,
				"validatorscount": 7,
	...
	println!("   ✅ Mock response added for getaep17balances");

	// Mock getbalance response for account with tokens
	mock.add_response(
		"getaep17balances_with_tokens",
		json!({
			"balance": [
				{
	...
	println!("   ✅ Mock response added for account with ATC/GAS");
	...
	// Test empty balance
	match mock.mock_call("getaep17balances") {
		Ok(response) => {
	...
	match mock.mock_call("getaep17balances_with_tokens") {
		Ok(response) => {
			if let Some(balance) = response.get("balance") {
				let empty_vec = vec![];
				let balance_array = balance.as_array().unwrap_or(&empty_vec);
				...
				for token in balance_array {
						...
						(token.get("assethash"), token.get("amount"))
					{
						...
						let token_name = match hash_str {
							"0xef4073a0f2b305a38ec4050e4d3d28bc40ea63f5" => "ATC",
