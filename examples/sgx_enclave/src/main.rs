...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Atipicial SGX Enclave Example");
    println!("========================");
    ...
    let transaction_data = b"ATC transaction data to sign";
    let mut signature = [0u8; 64];
    
    let result = unsafe {
        atipicial_sign_transaction(
            enclave.geteid(),
            &mut retval,
    ...
    let data_to_hash = b"Atipicial blockchain data";
    let mut hash = [0u8; 32];
    
    let result = unsafe {
        atipicial_calculate_hash(
            enclave.geteid(),
            &mut retval,
            data_to_hash.as_ptr(),
            data_to_hash.len(),
