//! Code generation module for Atipicial dApp templates
//!
...
impl ProjectTemplate {
	...
	pub fn display_name(&self) -> &str {
		match self {
			ProjectTemplate::BasicDapp => "Basic Atipicial dApp",
			ProjectTemplate::Aep17Token => "AEP-17 Token",
			ProjectTemplate::NftCollection => "NFT Collection (AEP-11)",
...
fn load_template(template_type: &ProjectTemplate) -> Result<Template> {
	...
	let template_content = match template_type {
		...
		ProjectTemplate::Aep17Token => {
			include_str!("../templates/aep17_token.toml")
		},
...
fn generate_file(
	...
	let content = content
		...
		.replace("{{atipicial_version}}", atipicial::VERSION);
...
fn print_next_steps(project_name: &str, template_type: &ProjectTemplate) {
	...
	match template_type {
		ProjectTemplate::Aep17Token => {
			println!("  4. Compile contract: atipicial-boa contracts/token.py");
			println!("  5. Deploy contract: atipicial-cli contract deploy");
		},
		ProjectTemplate::NftCollection => {
		...
		_ => {
			...
			println!("  5. Deploy to Atipicial blockchain");
		},
	}

	println!("\n📚 Documentation: {}", "https://github.com/R3E-Network/AtipicialRust".blue());
}
...
pub fn list_templates() {
	...
	let templates = vec![
		...
		(ProjectTemplate::Aep17Token, "Fungible token following AEP-17 standard"),
		(ProjectTemplate::NftCollection, "Non-fungible token collection (AEP-11)"),
...
mod tests {
	...
	fn test_embedded_templates_parse() {
		...
		let aep17 =
			load_template(&ProjectTemplate::Aep17Token).expect("aep17_token.toml should parse");
		assert!(aep17.files.contains_key("Cargo.toml"), "aep17 template should include Cargo.toml");
		assert!(
			aep17.files.contains_key("contracts/token.py"),
			"aep17 template should include contracts/token.py"
		);
	...
	fn test_project_generation() {
		...
		assert!(
			!cargo_contents.contains("{{atipicial_version}}"),
			"Generated Cargo.toml should not contain unresolved template variables"
		);
		assert!(
			cargo_contents.contains(&format!("atipicial = \"{}\"", atipicial::VERSION)),
			"Generated Cargo.toml should pin atipicial dependency to the current SDK version"
		);
	}
