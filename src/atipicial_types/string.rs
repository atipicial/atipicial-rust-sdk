...
use crate::{
	config::DEFAULT_ADDRESS_VERSION, atipicial_crypto::try_base58check_decode, atipicial_types::TypeError,
};
use atipicial::prelude::ScriptHash;
...
mod tests {
	...
	fn test_try_base58_decoded_rejects_invalid_input() {
		...
		assert!(matches!(
			...
			Err(crate::atipicial_types::TypeError::InvalidFormat(message)) if message.contains("base58")
		));
	}
	...
	fn test_try_base58_check_decoded_rejects_invalid_checksum() {
		...
		assert!(matches!(
			...
			Err(crate::atipicial_types::TypeError::InvalidFormat(message)) if message.contains("base58check")
		));
	}
