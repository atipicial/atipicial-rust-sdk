...
use crate::{
	...
	atipicial_contract::{checked_vm_integer, traits::SmartContractTrait, ContractError, AtipicialIterator},
	atipicial_types::{
...
impl<'a, P: JsonRpcProvider + 'static> PolicyContract<'a, P> {
	...
	pub async fn get_blocked_accounts(&self) -> Result<AtipicialIterator<'_, H160, P>, ContractError> {
	...
	pub async fn get_whitelist_fee_contracts(
		&self,
	) -> Result<AtipicialIterator<'_, WhitelistedContract, P>, ContractError> {
		self.call_function_returning_iterator(
	...
	async fn collect_all<T>(
		&self,
		iterator: AtipicialIterator<'_, T, P>,
		batch_size: i32,
	...
	/// * `token` - The token contract to recover (must implement AEP-17).
...
mod tests {
	...
	use crate::{
		...
		atipicial_types::{InvocationResult, AtipicialVMStateType, StackItem},
	};
	use serde_json::json;
