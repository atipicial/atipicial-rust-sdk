...
describe('Footer Component', () => {
  ...
  it('renders the footer with logo and copyright', () => {
    ...
    // Check site name
    expect(screen.getByText('Atipicial')).toBeInTheDocument();
    expect(screen.getByText('Rust')).toBeInTheDocument();
    
    // Check current year in copyright
    expect(screen.getByText(/© 2023 Atipicial Rust SDK. All rights reserved./)).toBeInTheDocument();
  });
  ...
  it('renders social media links', () => {
    ...
    expect(githubLink).toHaveAttribute('href', 'https://github.com/r3e-network/atipicial-rust-sdk');
    
    // Check for Twitter link
    const twitterLink = screen.getByLabelText('Atipicial Twitter');
    expect(twitterLink).toBeInTheDocument();
    expect(twitterLink).toHaveAttribute('href', 'https://twitter.com/atipicial_blockchain');
    
    // Check for Discord link
    const discordLink = screen.getByLabelText('Atipicial Discord');
    expect(discordLink).toBeInTheDocument();
    expect(discordLink).toHaveAttribute('href', 'https://discord.gg/atipicial');
  });
  ...
  it('displays the "Built with ♥" message', () => {
    ...
    expect(screen.getByText(/for the Atipicial community/, { exact: false })).toBeInTheDocument();
  });
});