...
use crate::{atipicial_wallets::AEP6Account, ScryptParamsDef};
use getset::{CopyGetters, Getters};
use serde::{Deserialize, Serialize};

/// Represents a AEP-6 wallet.
...
impl Aep6Wallet {
	/// Creates a new AEP-6 wallet with the given parameters.
	///
	/// # Arguments
	///
	...
	///
	/// let wallet = wallets::Aep6Wallet::new(name, version, scrypt, accounts, extra);
	/// ```
	pub fn new(
		name: String,
		version: String,
		scrypt: ScryptParamsDef,
		accounts: Vec<AEP6Account>,
...
#[cfg(test)]
mod tests {
	use crate::{atipicial_wallets::Aep6Wallet, ScryptParamsDef};
	use atipicial::prelude::ContractParameterType;

	#[test]
	fn test_read_wallet() {
		let data = include_str!("../../../test_resources/wallet/wallet.json");
		let wallet: Aep6Wallet = serde_json::from_str(data)
			.expect("Should be able to deserialize valid Aep6Wallet JSON in test");
		...
		let parameter1 = &contract1.aep6_parameters[0];
		...
		let parameter2 = &contract2.aep6_parameters[0];
