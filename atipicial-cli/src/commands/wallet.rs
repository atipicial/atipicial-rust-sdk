...
pub enum WalletCommands {
	/// Create a new wallet
	#[command(about = "Create a new Atipicial wallet")]
	Create {
	...
	ImportAddress {
		/// Atipicial address to import
		#[arg(short, long, help = "Atipicial address")]
		address: String,
	...
	Send {
		/// Asset ID (ATC, GAS, or script hash)
		#[arg(short, long, help = "Asset to transfer (ATC, GAS, or token hash)")]
		asset: String,
	...
	Balance {
		...
		/// Only show this token (ATC, GAS, or script hash)
...
async fn handle_create_wallet(
	...
	println!("{table}");
	print_info("💡 Use 'atipicial-cli wallet create-address' to create your first address");
...
async fn handle_list_addresses(state: &CliState) -> Result<(), CliError> {
	...
	if wallet.accounts.is_empty() {
		...
		print_info("💡 Use 'atipicial-cli wallet create-address' to create an address");
		return Ok(());
	}
...
async fn handle_import_address(
	...
	address
		...
		.map_err(|e| CliError::InvalidInput(format!("Invalid Atipicial address '{address}': {e}")))?;
	...
	let mut account =
		if let Some(address) = address {
			let script_hash = address.address_to_script_hash().map_err(|e| {
				CliError::InvalidInput(format!("Invalid Atipicial address '{address}': {e}"))
			})?;
...
async fn handle_transfer(
	...
	let extra_network_fee = fee
		.as_deref()
		.map(|fee| {
			let amount = atipicial::sdk::DecimalAmount::parse(fee, 8)
...
async fn handle_balance(
	...
	for address in addresses {
		...
		let balances = rpc_client
			.get_aep17_balances(script_hash)
			.await
		...
		for bal in balances.balances {
			...
			let amount = atipicial::sdk::DecimalAmount::try_from_raw(bal.amount.clone(), decimals)
				.map_err(|err| {
					CliError::Rpc(format!(
...
async fn handle_restore_wallet(path: PathBuf, state: &mut CliState) -> Result<(), CliError> {
	...
	// Backups are valid AEP-6 wallets; restore by opening the backup file with a password.
...
async fn handle_hd_wallet(
	...
	use atipicial::sdk::hd_wallet::HDWallet;
...
async fn handle_websocket_subscribe(
	...
	let ws_url = url.unwrap_or_else(|| {
		if let Some(network) = &state.current_network {
			...
			"wss://testnet1.atipicial.coz.io:443/ws".to_string()
		}
	});
	...
	match simulator.simulate_transaction(&script_bytes, signer_list).await {
		Ok(result) => {
			...
			table.add_row(vec![
				...
				Cell::new(if result.vm_state == AtipicialVMStateType::Halt {
					"SUCCESS"
				} else {
					"FAILED"
				})
				.fg(if result.vm_state == AtipicialVMStateType::Halt {
					Color::Green
				} else {
					Color::Red
				}),
