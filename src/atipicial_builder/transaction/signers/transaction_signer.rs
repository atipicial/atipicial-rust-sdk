...
use crate::{
	...
	atipicial_types::{deserialize_vec_public_key_option, serialize_vec_public_key_option},
...
/// Represents a transaction signer in the ATC blockchain.
///
...
impl AtipicialSerializable for TransactionSigner {
	type Error = TransactionError;

	/// Calculates the size of the serialized `TransactionSigner`.
	fn size(&self) -> usize {
		let mut size = (AtipicialConstants::HASH160_SIZE + 1) as usize;
