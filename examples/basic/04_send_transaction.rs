...
## What you'll learn:
- Creating ATC and GAS transfer transactions
...
## Prerequisites:
- Test wallet with ATC/GAS balance on TestNet
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	println!("{}", "💸 AtipicialRust Basic Transaction Example".cyan().bold());
	println!("{}", "=".repeat(50));
...
async fn check_balance(client: &RpcClient<HttpProvider>, address: &str) {
	print!("  🔶 ATC: ");
	match get_balance_with_timeout(client, address, "ATC").await {
		Ok(balance) => println!("{}", format!("{balance} ATC").cyan()),
...
async fn get_balance_with_timeout(
	...
	// Note: This is simplified for demo - real implementation would use get_aep17_balances
...
async fn demonstrate_transaction_building(
	...
	println!("  📋 Building ATC transfer transaction...");
	...
	let amount = 1u64; // 1 ATC
	println!("    📊 Transfer Amount: {} ATC", amount.to_string().cyan());
...
async fn demonstrate_gas_estimation(client: &RpcClient<HttpProvider>) {
	...
	println!("\n  📊 Typical Gas Fees:");
	println!("    • Simple ATC transfer: ~0.00000001 GAS");
...
mod tests {
	...
	fn test_wallet_generation() {
		...
		// Address should be valid Atipicial format
