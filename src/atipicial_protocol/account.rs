//! # Atipicial Account Module
//!
//! The Account module provides functionality for managing Atipicial accounts, including
//! key management, signature operations, and interaction with the Atipicial blockchain.
//!
//! ## Overview
//!
//! This module implements account-related operations for the Atipicial blockchain, including:
//!
...
use crate::atipicial_crypto::utils::ToHexString;
use primitive_types::H160;
use serde::{Deserialize, Serialize};
use signature::{hazmat::PrehashSigner, Error};

use crate::{
	atipicial_builder::VerificationScript,
	atipicial_clients::{public_key_to_address, APITrait, JsonRpcProvider, ProviderError, RpcClient},
	atipicial_crypto::{private_key_from_wif, KeyPair, Secp256r1PublicKey, Secp256r1Signature},
	atipicial_protocol::{get_aep2_from_private_key, get_private_key_from_aep2, AEP2},
	atipicial_types::{
	...
	atipicial_wallets::{AEP6Account, AEP6Contract, AEP6Parameter, Wallet},
...
impl AccountTrait for Account {
	...
	fn decrypt_private_key(&mut self, password: &str) -> Result<(), Self::Error> {
		...
		let key_pair = get_private_key_from_aep2(encrypted_private_key, password).map_err(|e| {
		...
		// without an address or verification script in their AEP-6 representation.
	...
	fn encrypt_private_key(&mut self, password: &str) -> Result<(), Self::Error> {
		...
		let mut hex_key = key_pair
			...
			.to_hex_string();
		let encrypted_private_key = get_aep2_from_private_key(&hex_key, password)
...
impl Account {
	...
	pub fn decrypt_private_key_with_params(
		...
		let key_pair = AEP2::decrypt_with_params(password, encrypted_private_key, *params)
			.map_err(|e| {
	...
	pub fn encrypt_private_key_with_params(
		...
		let encrypted_private_key = AEP2::encrypt_with_params(password, key_pair, *params)
			.map_err(|e| {
	...
	pub fn to_aep6_account(&self) -> Result<AEP6Account, ProviderError> {
		...
		let script_data = self.verification_script.as_ref().ok_or_else(|| {
			ProviderError::IllegalState(
				"verification_script unexpectedly None in to_aep6_account".to_string(),
			)
		})?;
	...
	pub async fn get_aep17_balances<P>(
		&self,
	...
	{
		let response =
			provider.get_aep17_balances(self.address_or_scripthash().script_hash()).await?;
...
mod tests {
	...
	use super::*;
	use crate::atipicial_config::TestConstants;
