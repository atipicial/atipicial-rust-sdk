use atipicial::atipicial_fs::{
	client::{
		AtipicialFsAuth, AtipicialFsClient, AtipicialFsConfig, DEFAULT_TESTNET_HTTP_GATEWAY, DEFAULT_TESTNET_REST_API,
	},
	container::Container,
	object::Object,
	types::{ContainerId, OwnerId},
	AtipicialFsService, ObjectType,
};
use reqwest::Client;
use std::env;
use uuid::Uuid;

/// Minimal, runnable AtipicialFs example that builds real request payloads and probes the public TestNet
/// gateway. If `ATCFS_WALLET` is set, it will also attempt a best-effort container listing.
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	println!("📦 Atipicial AtipicialFs Basic Usage (TestNet)");
	println!("======================================");

	let endpoint =
		env::var("ATCFS_ENDPOINT").unwrap_or_else(|_| DEFAULT_TESTNET_REST_API.to_string());
	let wallet = env::var("ATCFS_WALLET").ok();

	let auth = wallet
		.as_ref()
		.map(|addr| AtipicialFsAuth { wallet_address: addr.clone(), private_key: None });
	let mut config_builder = AtipicialFsConfig::builder()
	...
	let client = AtipicialFsClient::new(config.clone());
	...
	println!("\n✅ AtipicialFs basic usage demo completed.");
	Ok(())
}
