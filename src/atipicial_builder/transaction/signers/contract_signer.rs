...
use crate::{
	...
	atipicial_types::{deserialize_vec_public_key, serialize_vec_public_key},
...
/// Represents a contract signer in the ATC blockchain.
///
...
impl AtipicialSerializable for ContractSigner {
	type Error = TransactionError;

	fn size(&self) -> usize {
		let mut size: usize = AtipicialConstants::HASH160_SIZE as usize + 1; // +1 for scope byte
	...
	where
		Self: Sized,
	{
		fn read_bounded_list<T: AtipicialSerializable>(
			reader: &mut Decoder,
			max_len: usize,
			item_name: &str,
