...
Atipicial provides standard interfaces for token contracts, similar to Ethereum's ERC standards. These standards ensure that tokens on the Atipicial blockchain maintain consistent behavior and compatibility with wallets, exchanges, and other applications.

## AEP-17: Fungible Tokens

AEP-17 is the standard for fungible tokens on Atipicial, similar to Ethereum's ERC-20 standard. Fungible tokens are interchangeable with each other, like currencies.
...
### Usage with AtipicialRust

```rust,no_run
use atipicial::prelude::*;
use atipicial::atipicial_contract::FungibleTokenContract;
use std::str::FromStr;

async fn aep17_example() -> Result<(), Box<dyn std::error::Error>> {
    // Connect to Atipicial
    let provider = HttpProvider::new("https://mainnet1.atipicial.com:443")?;
    let client = RpcClient::new(provider);
    
    // Create a reference to a AEP-17 token (GAS token used as example)
...
### Built-in AEP-17 Tokens

Atipicial includes two native AEP-17 tokens:

#### ATC Token

The ATC token represents ownership in the Atipicial network and grants voting rights:

```rust,no_run
let atipicial_token = AtipicialCoin::new(&client);
let balance = atipicial_token.balance_of(&account_hash).await?;
let voting_power = atipicial_token.get_unclaimed_gas(&account_hash, block_height).await?;
```
...
AEP-11 is the standard for non-fungible tokens on Atipicial, similar to Ethereum's ERC-721 standard. Non-fungible tokens represent unique assets.
...
```rust,no_run
use atipicial::prelude::*;
use atipicial::atipicial_contract::NftContract;
use std::str::FromStr;

async fn aep11_example() -> Result<(), Box<dyn std::error::Error>> {
    // Connect to Atipicial
    let provider = HttpProvider::new("https://mainnet1.atipicial.com:443")?;
    let client = RpcClient::new(provider);
    
    // Create a reference to a AEP-11 token
...
## AEP-11 Divisible Tokens

AEP-11 supports divisible NFTs, which can be partially owned by multiple accounts. These add additional methods to the standard:
...
Both AEP-17 and AEP-11 standards define events that must be emitted by compliant contracts:
...
To implement a custom token contract, you'll need to develop a Atipicial smart contract that complies with the relevant AEP standard. This requires:

1. Implementing all standard methods defined by the AEP
...
// For AEP-11 (number of tokens owned)
let balance = nft.balance_of(&account_hash).await?;

// For divisible AEP-11 (balance of specific token)
