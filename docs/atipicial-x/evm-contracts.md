...
Atipicial X supports Ethereum Virtual Machine (EVM) compatible smart contracts, allowing developers to deploy and interact with contracts written in Solidity or other EVM-compatible languages. This document covers the process of working with EVM contracts on Atipicial X using the AtipicialRust SDK.

## Contract Deployment

### Deploying a New Contract

To deploy a new EVM contract on Atipicial X:
...
async fn deploy_contract() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let deploy_tx = AtipicialXTransaction::new()
        .data(bytecode)
...
async fn use_contract_abi() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let contract = AtipicialXContract::new_with_abi(contract_address, abi, provider.clone())?;
...
async fn proxy_contract_interaction() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let implementation = AtipicialXContract::new(proxy_address, provider.clone())
    ...
    let contract = AtipicialXContract::new_with_abi(proxy_address, abi, provider.clone())?;
...
For testing before deploying to Atipicial X mainnet:

```rust,no_run
async fn test_with_local_node() -> Result<(), Box<dyn std::error::Error>> {
    // Connect to local development node
    let provider = AtipicialXProvider::new_http("http://localhost:8545");
