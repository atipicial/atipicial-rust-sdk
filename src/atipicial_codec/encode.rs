...
impl AtipicialSerializable for H160 {
	...
	where
		Self: Sized,
	{
		let bytes = reader.read_bytes(AtipicialConstants::HASH160_SIZE as usize)?;
		<H160 as crate::atipicial_types::ScriptHashExtension>::from_slice(&bytes)
...
impl AtipicialSerializable for H256 {
	type Error = CodecError;
	...
	where
		Self: Sized,
	{
		let bytes = reader.read_bytes(AtipicialConstants::HASH256_SIZE as usize)?;
		if bytes.len() != 32 {
...
impl AtipicialSerializable for u8 {
	type Error = CodecError;
...
impl<T: AtipicialSerializable> VarSizeTrait for Vec<T> {
	fn var_size(&self) -> usize {
...
impl<T: AtipicialSerializable> VarSizeTrait for &[T] {
	fn var_size(&self) -> usize {
		let count_var_size = Self::get_var_size(self.len());
		count_var_size + self.iter().map(|item| item.size()).sum::<usize>()
	}
...
// fn var_size_of_serializables<T: AtipicialSerializable>(elements: &[T]) -> usize {
