...
This tutorial covers creating and sending transactions on the Atipicial blockchain using the AtipicialRust SDK.

## Understanding Atipicial Transactions

Atipicial transactions are the fundamental units of work in the Atipicial blockchain. They represent operations such as token transfers, smart contract invocations, and more. Each transaction has the following key components:
...
## Transferring ATC or GAS

The AtipicialRust SDK provides convenient methods for transferring ATC and GAS tokens:
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Similarly for ATC token
    let atipicial_token = AtipicialCoin::new(provider.clone());
    
    // Transfer 1 ATC to another address
...
Atipicial supports multi-signature accounts, which require multiple signatures to authorize a transaction:
...
Atipicial transactions require two types of fees:
