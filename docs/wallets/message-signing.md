...
Message signing is a cryptographic process that enables users to prove ownership of a private key without revealing it. In the Atipicial ecosystem, message signing serves several critical functions:
...
The message signing process in Atipicial follows these steps:
...
## Implementation in AtipicialRust

AtipicialRust provides comprehensive support for message signing through the `WalletSigner` struct:

```rust,no_run
use atipicial::prelude::*;
use atipicial::atipicial_wallets::WalletSigner;
...
- `Account`: Represents Atipicial accounts and their capabilities
