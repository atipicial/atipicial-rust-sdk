...
Invoking smart contracts is a fundamental operation when interacting with the Atipicial blockchain. The AtipicialRust SDK provides multiple approaches to call contract methods, both for state-changing operations (which require transactions) and read-only queries.

## Invocation Methods

### 1. Using Contract-Specific Interfaces

For common contracts like AEP-17 tokens or system contracts, the SDK provides dedicated interfaces:

```rust,no_run
use atipicial::prelude::*;
use atipicial::atipicial_contract::{FungibleTokenContract, FungibleTokenTrait, GasToken, TokenTrait};
use std::str::FromStr;
...
async fn token_interaction() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Using the generic AEP-17 interface for any fungible token
...
```rust,no_run
use atipicial::prelude::*;
use atipicial::atipicial_contract::{FungibleTokenTrait, GasToken, TokenTrait};
use std::str::FromStr;
...
```rust,no_run
use atipicial::prelude::*;
use atipicial::atipicial_contract::{FungibleTokenTrait, GasToken};
use std::str::FromStr;
...
```rust,no_run
use atipicial::prelude::*;
use atipicial::atipicial_builder::{ScriptBuilder, TransactionBuilder};
use std::str::FromStr;
...
async fn event_subscription() -> Result<(), Box<dyn std::error::Error>> {
    // Connect to Atipicial with WebSocket
    let provider = WebSocketProvider::new("wss://testnet1.atipicial.com:4443").await?;
    let client = RpcClient::new(provider);
