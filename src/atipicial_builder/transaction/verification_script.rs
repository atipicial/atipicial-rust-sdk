...
use crate::{
	crypto::Secp256r1PublicKey, atipicial_crypto::utils::ToHexString, atipicial_types::OpCode, prelude::Bytes,
};
...
impl VerificationScript {
	...
	pub fn is_multi_sig(&self) -> bool {
		...
		if !(1..=AtipicialConstants::MAX_PUBLIC_KEYS_PER_MULTI_SIG).contains(&threshold_u32) {
			return false;
		}
		...
		if !(m >= threshold && m <= BigInt::from(AtipicialConstants::MAX_PUBLIC_KEYS_PER_MULTI_SIG)) {
			return false;
		}
	...
	pub fn try_encode(&self, writer: &mut Encoder) -> Result<(), BuilderError> {
		if self.script.len() > AtipicialConstants::MAX_TRANSACTION_SIZE as usize {
			return Err(BuilderError::InvalidScript(format!(
				"verification script exceeds maximum transaction size of {} bytes",
				AtipicialConstants::MAX_TRANSACTION_SIZE
			)));
		}
...
impl AtipicialSerializable for VerificationScript {
...
mod tests {
	...
	use super::*;
	use crate::atipicial_crypto::utils::FromHexString;
	...
	fn test_try_to_array_rejects_oversized_script() {
		let script =
			VerificationScript::from(vec![0_u8; AtipicialConstants::MAX_TRANSACTION_SIZE as usize + 1]);
