...
use crate::atipicial_clients::{Authorization, JsonRpcProvider, ProviderError};
use atipicial::config::AtipicialConstants;
...
///
/// The variants remain exhaustive for compatibility with AtipicialRust SDK 2.x clients.
///
/// ```no_run
/// use atipicial::atipicial_clients::HttpClientError;
///
...
impl JsonRpcProvider for HttpProvider {
	...
	async fn fetch<T: Serialize + Send + Sync, R: DeserializeOwned>(
		...
		let max_response_size = AtipicialConstants::max_rpc_message_size();
...
impl Default for HttpProvider {
	...
	///
	/// # Panics
	/// Panics if AtipicialConstants::SEED_1 is not a valid URL. This is a compile-time
	/// constant and should always be valid.
	fn default() -> Self {
		let url = Url::parse(AtipicialConstants::SEED_1).unwrap_or_else(|e| {
			panic!(
				"AtipicialConstants::SEED_1 ('{}') is not a valid URL: {}. \
...
impl HttpProvider {
	...
	///
	/// ```
	/// use atipicial::atipicial_clients::{HttpProvider, Authorization};
	/// use url::Url;
	///
	/// let url = Url::parse("http://localhost:10332").unwrap();
...
mod tests {
	...
	async fn rejects_oversized_content_length_without_reading_body() {
		...
		let max = AtipicialConstants::max_rpc_message_size();
