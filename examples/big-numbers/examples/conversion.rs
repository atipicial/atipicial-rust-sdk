...
/// Atipicial Big Number Conversion Example
///
/// This example demonstrates how to convert between different number types
/// in Atipicial contexts, including token amounts, gas values, and blockchain data.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	println!("🔄 Atipicial Big Number Conversion Example");
	println!("======================================");

	// 1. Basic U256 conversions for Atipicial blockchain values
	println!("\n1️⃣ Basic U256 Conversions");

	// Start with a typical ATC token amount (1000 ATC with 8 decimals)
	let atipicial_amount = U256::from(1000u64) * U256::from(100_000_000u64);
	println!("   💎 ATC Amount (raw): {atipicial_amount}");

	// Safe conversions to smaller types
	let as_u128: u128 = atipicial_amount.as_u128();
	let as_u64: u64 = atipicial_amount.as_u64();
	let as_string: String = atipicial_amount.to_string();
	...
	let human_readable = convert_to_token_amount(atipicial_amount, 8);
	println!("   👤 Human readable: {human_readable} ATC");
	...
	// 3. Real Atipicial network data conversion (env-gated)
	println!("\n3️⃣ Real Atipicial Network Data Conversion");
	...
	if let Ok(rpc_url) = std::env::var("ATC_RPC_URL") {
		...
		if let Ok(_block_count) = client.get_block_count().await {
			println!("   ✅ Connected to Atipicial RPC");
		...
		println!("   ℹ️ Set ATC_RPC_URL to run the live network conversion (skipped).");
	}
	...
	println!("💡 These conversion patterns ensure safe handling of Atipicial values");
