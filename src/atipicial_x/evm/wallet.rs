...
/// Atipicial X EVM Wallet for managing accounts and signing transactions on Atipicial X.
...
impl AtipicialXWallet {
	/// Creates a new Atipicial X wallet from a hex-encoded private key string
	pub fn from_private_key(hex_key: &str) -> Result<Self, LocalSignerError> {
		let wallet = PrivateKeySigner::from_str(hex_key)?;
		Ok(Self { inner: wallet })
	}

	/// Creates a new Atipicial X wallet from randomly generated bytes
...
/// A wrapper to easily send EVM transactions on Atipicial X using a AtipicialXWallet and AtipicialXProvider.
pub struct AtipicialXClient<'a, P: JsonRpcProvider> {
	pub wallet: AtipicialXWallet,
	pub provider: AtipicialXProvider<'a, P>,
}

impl<'a, P: JsonRpcProvider + 'static> AtipicialXClient<'a, P> {
	/// Creates a new Atipicial X EVM client
	pub fn new(wallet: AtipicialXWallet, provider: AtipicialXProvider<'a, P>) -> Self {
		Self { wallet, provider }
	}

	/// Gets the balance of the wallet's address on Atipicial X
	...
	/// Sends a `AtipicialXTransaction` using the configured wallet and provider
	pub async fn send_transaction(
		&self,
		tx: AtipicialXTransaction,
	...
	/// [`AtipicialXTransaction`] representation supports.
	pub async fn send_transaction_request(
		...
		let url = self
			...
			.rpc_url()
			.parse()
			.map_err(|e: url::ParseError| format!("Invalid Atipicial X RPC URL: {e}"))?;
