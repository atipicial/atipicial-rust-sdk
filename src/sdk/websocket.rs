...
//!
//! This module provides comprehensive WebSocket connectivity for Atipicial blockchain,
...
//!
//! ```rust,no_run
//! use atipicial::sdk::websocket::{WebSocketClient, SubscriptionType};
//!
//! #[tokio::main]
...
use crate::atipicial_types::{Address, ScriptHash};
...
///
/// The main client for establishing WebSocket connections to Atipicial nodes and
...
impl WebSocketClient {
	...
	pub async fn new(url: &str) -> Result<Self, AtipicialError> {
		// Validate the URL format
		if !url.starts_with("ws://") && !url.starts_with("wss://") {
			return Err(AtipicialError::Network {
				...
				recovery: ErrorRecovery::new()
					...
					.doc("https://docs.atipicial.com/docs/n3/develop/tool/sdk/websocket"),
			});
		}
	...
	pub async fn connect(&mut self) -> Result<(), AtipicialError> {
		...
		let connect_result = if let Some(timeout) = AtipicialConstants::rpc_request_timeout() {
		...
		let (ws_stream, _) = connect_result.map_err(|e| AtipicialError::Network {
	...
	pub async fn disconnect(&mut self) -> Result<(), AtipicialError> {
		let Some(tx) = self.command_tx.take() else {
			return Ok(());
		};

		tx.send(Command::Shutdown).map_err(|e| AtipicialError::Network {
	...
	pub async fn subscribe(
		&mut self,
		subscription_type: SubscriptionType,
	) -> Result<SubscriptionHandle, AtipicialError> {
	...
	pub async fn unsubscribe(&mut self, handle: SubscriptionHandle) -> Result<(), AtipicialError> {
				...
				tokio::select! { biased;
					...
					next = ws_read.next() => {
						...
						if reconnect_attempts < max_reconnect_attempts {
							...
							let connect_result =
								if let Some(timeout) = AtipicialConstants::rpc_request_timeout() {
									match tokio::time::timeout(timeout, connect_fut).await {
										Ok(res) => res,
										Err(_) => {
											tracing::warn!(
	...
	async fn process_text_message(
		...
		let json: serde_json::Value =
			serde_json::from_str(text).map_err(|e| AtipicialError::Network {
	...
	async fn parse_event(json: &serde_json::Value) -> Result<Option<EventData>, AtipicialError> {
		let event_type = json.get("type").and_then(|t| t.as_str()).unwrap_or("");

		let required_str = |field: &str| -> Result<String, AtipicialError> {
		...
		let required_u32 = |field: &str| -> Result<u32, AtipicialError> {
			json.get(field)
				.and_then(|v| v.as_u64())
				.and_then(|v| u32::try_from(v).ok())
				.ok_or_else(|| AtipicialError::Network {
					message: format!(
						"WebSocket event '{}' missing or invalid '{}' field",
						event_type, field
					),
		...
		let required_u64 = |field: &str| -> Result<u64, AtipicialError> {
			json.get(field).and_then(|v| v.as_u64()).ok_or_else(|| AtipicialError::Network {
				message: format!(
					"WebSocket event '{}' missing or invalid '{}' field",
					event_type, field
				),
		...
		let event_data = match event_type {
			"block_added" => Some(EventData::NewBlock {
				...
				transactions: json
					...
					.iter()
					.map(|value| {
						value.as_str().map(str::to_string).ok_or_else(|| AtipicialError::Network {
							message:
			...
			"notification" => Some(EventData::Notification {
				...
				state: json.get("state").cloned().ok_or_else(|| AtipicialError::Network {
	...
	async fn send_message(&mut self, message: String) -> Result<(), AtipicialError> {
		if message.len() > AtipicialConstants::max_rpc_message_size() {
			return Err(AtipicialError::Network {
				message: "WebSocket message too large".to_string(),
				source: None,
				recovery: ErrorRecovery::new()
					.suggest("Reduce message size")
					.suggest("If needed, increase ATIPICIAL_MAX_RPC_MESSAGE_SIZE (bytes)"),
			});
		}
...
impl WebSocketClientBuilder {
	...
	pub async fn build(self) -> Result<WebSocketClient, AtipicialError> {
...
mod tests {
	...
	async fn subscribe_receives_event() {
		...
		WebSocketClient::process_text_message(
			&event,
			AtipicialConstants::max_rpc_message_size(),
