...
mod tests {
	...
	fn test_defi_swap_info() {
		...
		// Test getting swap information between ATC and GAS
	...
	fn test_defi_token_info() {
		...
		// Test ATC token info
		let output = cli.run_command(&["de-fi", "token", "ATC"]);
	...
	fn test_defi_invoke_test() {
		...
		// Test invoke ATC token symbol method (view only)
		let output = cli.run_command(&[
			"de-fi",
			"invoke",
			"ATC Token",
