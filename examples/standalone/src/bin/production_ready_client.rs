use atipicial::atipicial_clients::{
	CacheConfig, CircuitBreakerConfig, PoolConfig, ProductionClientConfig, ProductionRpcClient,
};
use std::time::Duration;

/// Example demonstrating production-ready Atipicial RPC client with all advanced features
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	// Initialize logging
	tracing_subscriber::fmt::init();

	println!("🚀 Atipicial Production-Ready RPC Client Example");
	println!("============================================\n");
	...
	// Get ATC token contract state (cached for 1 minute)
	let atipicial_token_hash = "0xef4073a0f2b305a38ec4050e4d3d28bc40ea63f5"; // ATC token on TestNet
	match client.get_contract_state(atipicial_token_hash.to_string()).await {
		Ok(contract_state) => {
	...
	match client.get_aep17_balances(test_address.to_string()).await {
		Ok(balances) => {
	...
	let failing_client = ProductionRpcClient::new(
		"https://nonexistent.atipicial.endpoint:443".to_string(),
...
mod tests {
	...
	async fn test_production_client_basic_operations() {
		let client = ProductionRpcClient::new(
			"https://testnet.atipicial.com:443".to_string(),
			ProductionClientConfig::default(),
		);
