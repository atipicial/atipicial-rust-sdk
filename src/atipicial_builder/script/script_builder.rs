use crate::{
	...
	atipicial_crypto::utils::{FromBase64String, FromHexString, ToHexString},
...
/// A builder for constructing Atipicial smart contract scripts.
///
...
/// required for Atipicial smart contract execution.
///
/// # Examples
///
/// ```rust
/// use atipicial::atipicial_builder::ScriptBuilder;
/// use atipicial::atipicial_types::OpCode;
/// use num_bigint::BigInt;
///
...
impl ScriptBuilder {
	...
	/// use atipicial::builder::CallFlags;
	///
	/// let mut builder = ScriptBuilder::new();
	...
	///     &contract_hash,
	///     "transfer",
	///     &[ContractParameter::from("AtipicialCoin"), ContractParameter::from(1000)],
	///     Some(CallFlags::All)
	/// );
	/// ```
	...
	/// ```rust
	/// use atipicial::atipicial_builder::ScriptBuilder;
	/// use atipicial::builder::InteropService;
	///
	/// let mut builder = ScriptBuilder::new();
	...
	///
	/// let mut builder = ScriptBuilder::new();
	/// builder.push_param(&ContractParameter::from("Hello, Atipicial!")).unwrap();
	/// ```
	...
	pub fn push_integer(&mut self, i: BigInt) -> &mut Self {
		self.try_push_integer(i).unwrap_or_else(|err| {
			panic!(
				"integer exceeds Atipicial VM 32-byte limit; use ScriptBuilder::try_push_integer for fallible handling: {}",
				err
			)
		})
	}
	...
	/// instead of truncating values that exceed the Atipicial VM 32-byte integer limit.
	///
	/// This method preserves Atipicial VM signed integer semantics. For example, `255` is encoded
		...
		match len {
			...
			_ => {
				return Err(BuilderError::IllegalArgument(
					"integer exceeds Atipicial VM 32-byte limit".to_string(),
				));
			},
		};
	...
	///
	/// let mut builder = ScriptBuilder::new();
	/// builder.push_data("Hello, Atipicial!".as_bytes().to_vec());
	/// ```
	...
	pub fn build_multi_sig_script(
		...
		// Canonical Atipicial sort: by compressed encoded bytes (via Ord impl)
		pubkeys.sort();
	...
	/// Builds a contract script for the given sender, AEF checksum, and contract name.
	///
	/// This method creates a script for deploying a smart contract on the Atipicial blockchain.
	///
	/// # Arguments
	///
	/// * `sender` - The 160-bit hash of the contract sender.
	/// * `aef_checksum` - The checksum of the AEF (Atipicial Executable Format) file.
	...
	/// let aef_checksum = 1234567890;
	/// let name = "MyContract";
	///
	/// let script = ScriptBuilder::build_contract_script(&sender, aef_checksum, name).unwrap();
	/// ```
	/// * `aef_checksum` - The checksum of the AEF file.
	/// * `name` - The name of the contract.
	///
	/// # Returns
	///
	...
	pub fn build_contract_script(
		...
		sb.op_code(&[OpCode::Abort])
			...
			.push_integer(BigInt::from(aef_checksum))
...
mod tests {
	use super::*;
	use crate::{
		atipicial_types::{contract::ContractParameterMap, ContractParameter, ContractParameterType},
		prelude::Bytes,
	};
	use num_bigint::BigInt;
	...
	fn test_push_string() {
		let mut builder = ScriptBuilder::new();
		let string_data = "Hello, Atipicial!";
	...
	#[test]
	#[should_panic(expected = "integer exceeds Atipicial VM 32-byte limit")]
