...
# Gas Estimation Guide - AtipicialRust

## Overview

AtipicialRust v2.0.0 includes real-time gas estimation using the Atipicial `invokescript` RPC method, providing accurate fee calculations for transactions before submission. This feature helps optimize costs and prevent transaction failures due to insufficient gas.

## Understanding Atipicial Gas

### Gas Types

Atipicial has two types of fees:
...
use atipicial::atipicial_types::Signer;

async fn estimate_transaction_gas(
    client: &ProductionAtipicialClient,
    script: &[u8],
...
async fn estimate_contract_deployment(
    ...
    let script = build_deployment_script(aef_file, manifest)?;
...
```rust
use atipicial::atipicial_errors::GasEstimationError;
...
impl GasEstimatorWithFallback {
    ...
    fn new(client: ProductionAtipicialClient) -> Self {
...
```rust
async fn monitor_gas_prices(client: &ProductionAtipicialClient) -> Result<(), Box<dyn Error>> {
...
mod tests {
    ...
    async fn test_gas_estimation() {
        ...
        // ATC balance check should cost around 0.01 GAS
        assert!(gas > 500_000);
...
The gas estimation feature in AtipicialRust v2.0.0 provides:
