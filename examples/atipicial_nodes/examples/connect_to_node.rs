...
/// This example demonstrates comprehensive Atipicial node connectivity and blockchain querying.
/// It includes connection handling, failover, performance monitoring, and various RPC operations.
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	println!("🌐 Atipicial Node Connection Example");
	println!("==================================\n");
	...
	let testnet_endpoints = vec![
		...
		"http://seed2t5.atipicial.com:20332",
		"http://seed3t5.atipicial.com:20332",
	];
	...
	for (name, hash) in native_contracts {
		match client.get_contract_state(atipicial::atipicial_types::ScriptHash::from_str(hash)?).await {
			Ok(state) => {
				println!(
					"   ✅ {}: v{}",
					name,
	...
	// Test MainNet
	print!("   🔷 MainNet: ");
	match test_network("https://mainnet1.atipicial.com:443/").await {
...
async fn test_network(endpoint: &str) -> Result<(u32, u128), Box<dyn std::error::Error>> {
	let start = Instant::now();
	let provider = atipicial::providers::HttpProvider::new(endpoint)?;
	let client = atipicial::providers::RpcClient::new(provider);
