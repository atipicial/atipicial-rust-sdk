...
use crate::{
	...
	atipicial_contract::ContractError,
};

/// Atipicial X MainNet Anti-MEV RPC endpoint.
pub const ATC_X_MAINNET_MEV_RPC: &str = "https://mainnet-1.rpc.banelabs.org";

/// Atipicial X EVM provider for interacting with the Atipicial X EVM-compatible chain.
///
...
impl<'a, P: JsonRpcProvider + 'static> AtipicialXProvider<'a, P> {
	/// Creates a new AtipicialXProvider instance with the specified RPC URL
	///
	/// # Arguments
	///
	/// * `rpc_url` - The RPC URL for the Atipicial X chain
	/// * `provider` - An optional reference to an RPC client (Atipicial)
	///
	/// # Returns
	///
	/// A new AtipicialXProvider instance
	...
	pub fn new_anti_mev(provider: Option<&'a RpcClient<P>>) -> Self {
		Self::new(ATC_X_MAINNET_MEV_RPC, provider)
	}

	/// Gets the RPC URL for the Atipicial X chain
	///
	/// # Returns
	///
	...
	/// Sets the RPC URL for the Atipicial X chain
	///
	/// # Arguments
	///
	...
	/// Gets the chain ID for the Atipicial X chain
	///
	/// # Returns
	///
	...
	pub async fn chain_id(&self) -> Result<u64, ContractError> {
		...
		let provider = self.provider.ok_or_else(|| {
			ContractError::ProviderNotSet(
				"Provider is required to query Atipicial X chain ID".to_string(),
			)
		})?;

		let value: Value =
			provider.request("atipicial_chainId", ()).await.map_err(ContractError::from)?;
		...
		match value {
				...
				if let Some(hex) = trimmed.strip_prefix("0x").or_else(|| trimmed.strip_prefix("0X"))
				{
					u64::from_str_radix(hex, 16).map_err(|_| {
						ContractError::InvalidArgError(format!(
							"Invalid Atipicial X chain ID hex value: {}",
							trimmed
						))
					})
				} else {
					trimmed.parse::<u64>().map_err(|_| {
						ContractError::InvalidArgError(format!(
							"Invalid Atipicial X chain ID value: {}",
							trimmed
						))
					})
				}
			...
			Value::Number(number) => number.as_u64().ok_or_else(|| {
				ContractError::InvalidArgError(
					"Atipicial X chain ID number is not representable as u64".to_string(),
				)
			}),
			other => Err(ContractError::InvalidArgError(format!(
				"Unexpected Atipicial X chain ID response: {}",
				other
			))),
		}
	}
...
mod tests {
	use super::*;
	use crate::atipicial_clients::{MockProvider, RpcClient};

	#[tokio::test]
	async fn chain_id_queries_provider() {
		let provider = MockProvider::new();
		provider.push_result("atipicial_chainId", Value::String("0xba93".to_string()));
		let client = RpcClient::new(provider);
		let provider = AtipicialXProvider::new("invalid://url", Some(&client));
	...
	async fn chain_id_requires_provider() {
		let provider: AtipicialXProvider<'_, MockProvider> = AtipicialXProvider::new("invalid://url", None);
	...
	fn parse_chain_id_value_rejects_invalid_strings() {
		let err =
			AtipicialXProvider::<MockProvider>::parse_chain_id_value(Value::String("wat".to_string()))
				.unwrap_err();
		assert!(
			matches!(err, ContractError::InvalidArgError(message) if message.contains("Invalid Atipicial X chain ID"))
		);
	}
}
