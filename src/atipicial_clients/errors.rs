...
use crate::{crypto::CryptoError, atipicial_clients::JsonRpcError, TypeError};
use thiserror::Error;
...
impl ProviderError {
	...
	/// Returns `true` when a Atipicial node does not know a polled transaction yet.
	pub fn is_unknown_transaction(&self) -> bool {
		matches!(self, Self::JsonRpcError(error) if error.is_unknown_transaction())
	}

	/// Returns `true` when a Atipicial node already has a submitted transaction.
