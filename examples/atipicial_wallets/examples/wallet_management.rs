...
/// This example demonstrates real wallet management functionality in Atipicial.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	for (index, account) in accounts.iter().take(2).enumerate() {
		...
		match client
			.invoke_function(
				&gas_token,
				"balanceOf".to_string(),
				vec![atipicial::atipicial_types::ContractParameter::h160(&account.get_script_hash())],
				None,
			)
			.await
		{
	...
	println!("\n✅ Atipicial wallet management example completed!");
