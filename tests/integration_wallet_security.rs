...
async fn test_complete_wallet_lifecycle() {
	...
	let original_addresses: Vec<String> =
		wallet.accounts().iter().map(atipicial::atipicial_protocol::Account::get_address).collect();
	let recovered_addresses: Vec<String> = recovered_wallet
		.accounts()
		.iter()
		.map(atipicial::atipicial_protocol::Account::get_address)
		.collect();
...
async fn test_large_wallet_performance() {
	...
	// With test mode enabled (ATCRUST_TEST_MODE=1), encryption should be much faster
