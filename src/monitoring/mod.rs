...
impl MonitoringConfig {
	...
	pub fn from_env() -> Self {
		Self {
			metrics_enabled: std::env::var("ATC_METRICS_ENABLED")
				.unwrap_or_else(|_| "true".to_string())
				.parse()
				.unwrap_or(true),
			metrics_port: std::env::var("ATC_METRICS_PORT")
				.unwrap_or_else(|_| "9090".to_string())
				.parse()
				.unwrap_or(9090),
			tracing_enabled: std::env::var("ATC_TRACING_ENABLED")
				.unwrap_or_else(|_| "true".to_string())
				.parse()
				.unwrap_or(true),
			tracing_endpoint: std::env::var("ATC_TRACING_ENDPOINT")
				.unwrap_or_else(|_| "http://localhost:4317".to_string()),
			log_level: std::env::var("ATC_LOG_LEVEL").unwrap_or_else(|_| "info".to_string()),
			health_check_enabled: std::env::var("ATC_HEALTH_CHECK_ENABLED")
				.unwrap_or_else(|_| "true".to_string())
				.parse()
				.unwrap_or(true),
			health_check_port: std::env::var("ATC_HEALTH_CHECK_PORT")
