...
describe('Blockchain Info Component', () => {
  ...
  it('fetches and displays blockchain information', async () => {
    ...
    // Check version
    expect(screen.getByText('/Atipicial:3.5.0/')).toBeInTheDocument(); // Version
  ...
  it('displays external link for blockchain data', async () => {
    ...
    // Look for an anchor tag with an href that contains 'explorer.atipicial.com/block/'
    const links = screen.getAllByRole('link');
    
    // Find the link that contains the block explorer URL
    const explorerLink = links.find(link => 
      link.getAttribute('href')?.includes('explorer.atipicial.com/block/')
    );
    ...
    expect(explorerLink).toHaveAttribute(
      'href', 
      expect.stringContaining('explorer.atipicial.com/block/')
    );
  });
