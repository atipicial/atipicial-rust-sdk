...
impl NNSName {
	...
	pub fn is_valid(name: &str, allow_multi_fragments: bool) -> Result<(), TypeError> {
		if name.len() < 3 || name.len() > 255 {
			return Err(TypeError::InvalidAtipicialName("Invalid name length".to_string()));
		}

		let fragments: Vec<&str> = name.split('.').collect();
		if fragments.len() < 2 || fragments.len() > 8 {
			return Err(TypeError::InvalidAtipicialName("Invalid fragment count".to_string()));
		}

		if fragments.len() > 2 && !allow_multi_fragments {
			return Err(TypeError::InvalidAtipicialName("Multiple fragments not allowed".to_string()));
		}
	...
	fn validate_fragment(fragment: &str, is_root: bool) -> Result<(), TypeError> {
		...
		if fragment.is_empty() || fragment.len() > max_len {
			return Err(TypeError::InvalidAtipicialName("Invalid fragment length".to_string()));
		}

		let first = fragment
			.chars()
			.next()
			.ok_or_else(|| TypeError::InvalidAtipicialName("Fragment cannot be empty".to_string()))?;
		if is_root && !first.is_ascii_alphabetic() {
			return Err(TypeError::InvalidAtipicialName("Root must start with letter".to_string()));
		} else if !(is_root || first.is_ascii_alphanumeric()) {
			return Err(TypeError::InvalidAtipicialName("Invalid start character".to_string()));
		}
		...
		if fragment[1..].chars().any(|c| !(c.is_ascii_alphanumeric() || c == '-')) {
			return Err(TypeError::InvalidAtipicialName("Invalid character in fragment".to_string()));
		}
		...
		if !(last.is_ascii_alphanumeric()) {
			return Err(TypeError::InvalidAtipicialName("Must end with alphanumeric".to_string()));
		}
