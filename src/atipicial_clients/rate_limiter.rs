...
use crate::atipicial_error::legacy_error::{AtipicialError, NetworkError};
use std::sync::Arc;
...
impl RateLimiter {
	...
	pub async fn acquire(&self) -> Result<RateLimitPermit<'_>, AtipicialError> {
		// First acquire semaphore permit for concurrency limiting
		let _sem_permit = self
			.semaphore
			.acquire()
			.await
			.map_err(|_| AtipicialError::Network(NetworkError::RateLimitExceeded))?;
	...
	pub async fn try_acquire(&self) -> Result<RateLimitPermit<'_>, AtipicialError> {
		...
		if bucket.tokens >= 1.0 {
			...
			Err(AtipicialError::Network(NetworkError::RateLimitExceeded))
		}
	}
