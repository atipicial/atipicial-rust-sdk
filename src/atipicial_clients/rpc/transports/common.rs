...
///
/// Inspired by atipicial-js logic:
/// <https://github.com/atipicial-io/atipicial.js/blob/9f990c57f0486728902d4b8e049536f2bb3487ee/packages/providers/src.ts/json-rpc-provider.ts#L25-L53>
...
impl JsonRpcError {
	...
	/// Returns `true` when a Atipicial node reports that a transaction is not known yet.
	///
	/// Current Atipicial nodes use `-103` for an unknown transaction and `-105` for an unknown script
	...
	/// Returns `true` when a Atipicial node reports that a submitted transaction is already known.
	///
	/// Atipicial uses `-501` when the transaction already exists in the ledger and `-503` when it is
	...
	/// Returns `true` for deterministic Atipicial transaction-submission rejections.
	pub fn is_transaction_rejection(&self) -> bool {
		matches!(self.code, -500 | -504 | -505 | -507 | -508 | -509 | -510 | -511 | -512)
	}

	/// Determine if the error output of the `atipicial_call` RPC request is a revert
	///
...
/// Claims struct as defined in <https://github.com/atipicial/execution-apis/blob/main/src/engine/authentication.md#jwt-claims>
...
mod tests {
	...
	#[test]
	fn ser_request() {
		let request: Request<()> = Request::new(0, "atipicial_chainId", ());
		assert_eq!(
			&serde_json::to_string(&request).unwrap(),
			r#"{"id":0,"jsonrpc":"2.0","method":"atipicial_chainId"}"#
		);
