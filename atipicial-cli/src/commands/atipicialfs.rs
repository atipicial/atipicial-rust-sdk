...
use crate::{
	...
	errors::CliError,
	utils::config::{load_config, save_config, AtipicialFsEndpoint},
...
// AtipicialFs endpoint constants
...
// Production-ready AtipicialFs client
struct AtipicialFsClient {
...
impl AtipicialFsClient {
	...
	async fn get_network_status(&self) -> Result<NetworkStatus, CliError> {
		...
		match self.http_client.get(&url).send().await {
			Ok(response) => Self::parse_json(response, "Failed to get AtipicialFs network status").await,
	...
	async fn list_containers(&self) -> Result<Vec<ContainerInfo>, CliError> {
		...
		match self.http_client.get(&url).send().await {
			Ok(response) => Self::parse_json(response, "Failed to list AtipicialFs containers").await,
			Err(e) => Err(CliError::Network(format!("Connection error: {}", e))),
		}
	}
	...
	async fn get_container(&self, container_id: &str) -> Result<ContainerInfo, CliError> {
		...
		match self.http_client.get(&url).send().await {
			Ok(response) => Self::parse_json(response, "Failed to get AtipicialFs container").await,
			Err(e) => Err(CliError::Network(format!("Connection error: {}", e))),
		}
	}
	...
	async fn list_objects(
		...
		match self.http_client.get(&url).send().await {
			Ok(response) => Self::parse_json(response, "Failed to list AtipicialFs objects").await,
			Err(e) => Err(CliError::Network(format!("Connection error: {}", e))),
		}
	}
	...
	async fn upload_object(
		...
		let boundary = format!("atipicial-cli-{}", uuid::Uuid::new_v4());
			...
			.await
		{
			Ok(response) => {
				let upload: UploadResponse =
					Self::parse_json(response, "Failed to upload AtipicialFs object").await?;
...
pub enum AtipicialFsCommands {
	...
	/// Show AtipicialFs network status
	Status,
}
...
pub enum ObjectCommands {
	/// Upload an object to AtipicialFs
	Put {
		/// Path to local file
		#[arg(short, long)]
		file: PathBuf,
	...
	/// Download an object from AtipicialFs
	Get {
		/// Container ID or name
		#[arg(short, long)]
		container: String,
...
/// Handle AtipicialFs commands
pub async fn handle_atipicialfs_command(args: AtipicialFsArgs, _state: &mut CliState) -> Result<(), CliError> {
	let client = match args.endpoint {
		Some(endpoint) => AtipicialFsClient::with_endpoint(&endpoint),
		None => configured_atipicialfs_client()?,
	};

	match args.command {
		AtipicialFsCommands::Container { command } => handle_container_command(command, &client).await,
		AtipicialFsCommands::Object { command } => handle_object_command(command, &client).await,
		AtipicialFsCommands::Acl { command } => handle_acl_command(command, &client).await,
		AtipicialFsCommands::Config { command } => handle_config_command(command).await,
		AtipicialFsCommands::Status => handle_status_command(&client).await,
	}
}

fn configured_atipicialfs_client() -> Result<AtipicialFsClient, CliError> {
	let config = load_config()?;
	let Some(default_name) = config.atipicialfs.default_endpoint else {
		return Ok(AtipicialFsClient::default());
	};

	let Some(endpoint) =
		config.atipicialfs.endpoints.iter().find(|endpoint| endpoint.name == default_name)
	else {
		return Err(CliError::Config(format!(
			"Default AtipicialFs endpoint '{}' is not present in the configured endpoint list",
			default_name
		)));
	};

	Ok(AtipicialFsClient::with_endpoint(&endpoint.url))
}

fn unsupported_signed_atipicialfs_operation(operation: &str) -> CliError {
	CliError::InvalidOperation(format!(
		"{operation} requires AtipicialFs native signed API support. The built-in HTTP gateway client \
		 supports object upload and download only, so the CLI refuses to report success for this \
		 operation without a signed AtipicialFs backend."
	))
}
...
async fn handle_container_command(
	...
	match command {
		ContainerCommands::Create { name, acl: _, options: _ } => {
			...
			Err(unsupported_signed_atipicialfs_operation("Container creation"))
		},
		ContainerCommands::List => {
			print_info("Retrieving containers from AtipicialFs network...");
		...
		ContainerCommands::Get { id } => {
			print_info(&format!("Retrieving container {} from AtipicialFs network...", id));
		...
		ContainerCommands::Delete { id, force } => {
			let _ = (id, force);
			Err(unsupported_signed_atipicialfs_operation("Container deletion"))
		},
	}
}
...
async fn handle_object_command(
	...
	match command {
		...
		ObjectCommands::Delete { container, object, force } => {
			...
			Err(unsupported_signed_atipicialfs_operation("Object deletion"))
		},
	}
}

/// Handle ACL commands
async fn handle_acl_command(command: AclCommands, client: &AtipicialFsClient) -> Result<(), CliError> {
	match command {
		AclCommands::Get { container } => {
			let _ = (container, client);
			Err(unsupported_signed_atipicialfs_operation("ACL retrieval"))
		},
		AclCommands::Set { container, rules } => {
			let _ = (container, rules, client);
			Err(unsupported_signed_atipicialfs_operation("ACL update"))
		},
	}
}
	...
	match command {
		ConfigCommands::SetEndpoint { url, env } => {
			let parsed = Url::parse(&url)
				.map_err(|e| CliError::InvalidInput(format!("Invalid AtipicialFs endpoint URL: {e}")))?;
			let scheme = parsed.scheme();
			if scheme != "http" && scheme != "https" {
				return Err(CliError::InvalidInput(
					"AtipicialFs endpoint URL must use http or https".to_string(),
				));
			}
			...
			let endpoint = AtipicialFsEndpoint {
				name: name.clone(),
				url: url.clone(),
				network,
			...
			if let Some(existing) =
				config.atipicialfs.endpoints.iter_mut().find(|existing| existing.name == name)
			{
				*existing = endpoint;
			} else {
				config.atipicialfs.endpoints.push(endpoint);
			}
			config.atipicialfs.default_endpoint = Some(name.clone());
			save_config(&config)?;

			print_success(&format!("Default AtipicialFs endpoint set to '{}' ({})", name, url));
			Ok(())
		},
		ConfigCommands::Get => {
			let config = load_config()?;
			print_info("Current AtipicialFs configuration:");
			println!(
				"Default Endpoint: {}",
				config.atipicialfs.default_endpoint.as_deref().unwrap_or("<none>")
			);
			for endpoint in config.atipicialfs.endpoints {
				println!(
					"- {} [{} {}]: {}",
...
async fn handle_status_command(client: &AtipicialFsClient) -> Result<(), CliError> {
	print_info(&format!("Checking AtipicialFs status on endpoint: {}", client.grpc_endpoint));
